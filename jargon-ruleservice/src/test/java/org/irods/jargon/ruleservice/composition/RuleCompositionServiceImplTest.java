...
public class RuleCompositionServiceImplTest {
	...
	public void testParseIrodsFileIntoRule() throws Exception {
		...
		Rule rule = ruleCompositionService.loadRuleFromIrods(targetIrodsCollection + "/" + irodsRuleFile);

		Assert.assertNotNull("null rule", rule);

		Assert.assertFalse("empty rule body", rule.getRuleBody().isEmpty());
		Assert.assertFalse("empty input parms", rule.getInputParameters().isEmpty());
		Assert.assertFalse("empty output parms", rule.getOutputParameters().isEmpty());
	...
	public void testStoreRuleFromParts() throws Exception {
		...
		Rule returnedFromStore = ruleCompositionService.storeRuleFromParts(irodsRuleFileAsFile.getAbsolutePath(),
				rule.getRuleBody(), inputParameters, outputParameters);
		...
		Assert.assertNotNull("rule not reloaded from iRODS", actual.getRuleBody());

		Assert.assertEquals("unequal number of input params", rule.getInputParameters().size(),
				actual.getInputParameters().size());
		Assert.assertEquals("unequal number of output params", rule.getOutputParameters().size(),
	...
	public void testStoreRule() throws Exception {
		...
		Rule returnedFromStore = ruleCompositionService.storeRule(irodsRuleFileAsFile.getAbsolutePath(), rule);
	...
	public void testStoreRuleOverwrite() throws Exception {
		...
		returnedFromStore = ruleCompositionService.storeRule(irodsRuleFileAsFile.getAbsolutePath(), rule);
		...
		Assert.assertNotNull("rule not reloaded from iRODS", actual);
		Assert.assertTrue("hello should be in rule body", actual.getRuleBody().indexOf("hello") > -1);
	...
	public void testDeleteInputParameterFromRule() throws Exception {
		...
		ruleCompositionService.storeRule(irodsRuleFileAsFile.getAbsolutePath(), rule);

		// delete first input parm

		ruleCompositionService.deleteInputParameterFromRule(irodsRuleFileAsFile.getAbsolutePath(), "*dataObject");
		...
		Assert.assertEquals("unequal number of input params", rule.getInputParameters().size() - 1,
				actual.getInputParameters().size());
		Assert.assertEquals("unequal number of output params", rule.getOutputParameters().size(),
				actual.getOutputParameters().size());
	...
	public void testDeleteOutputParameterFromRule() throws Exception {
		...
		ruleCompositionService.deleteOutputParameterFromRule(irodsRuleFileAsFile.getAbsolutePath(), "ruleExecOut");
	...
	public void testReadARuleFromFileWithOuputParams() throws Exception {
		...
		Assert.assertEquals("unequal number of output params", 1, rule.getOutputParameters().size());

		IRODSRuleParameter outputParam = rule.getOutputParameters().get(0);
		Assert.assertEquals("did not get *DestFile", "*DestFile", outputParam.getUniqueName());
	...
	public void testParseIrodsFileIntoRuleMissing() throws Exception {
		...
		ruleCompositionService.loadRuleFromIrods(targetIrodsCollection + "/" + irodsRuleFile);
	...
	public void testAddInputParameterToRule() throws Exception {
		...
		Rule actual = ruleCompositionService.addInputParameterToRule(irodsRuleFileAsFile.getAbsolutePath(),
				newParamName, newParamValue);

		Assert.assertNotNull("null rule returned", actual);

		Assert.assertEquals("expected 1 new param", nbrBefore + 1, actual.getInputParameters().size());

		boolean foundNew = false;

		for (int i = 0; i < actual.getInputParameters().size(); i++) {
			if (actual.getInputParameters().get(i).getUniqueName().equals(newParamName)) {
				foundNew = true;
				break;
			}
		}
	...
	public void testAddDuplicateInputParameterToRule() throws Exception {
		...
		ruleCompositionService.addInputParameterToRule(irodsRuleFileAsFile.getAbsolutePath(), newParamName,
				newParamValue);
		ruleCompositionService.addInputParameterToRule(irodsRuleFileAsFile.getAbsolutePath(), newParamName,
				newParamValue);
	...
	public void testAddOutputParameterToRule() throws Exception {
		...
		Rule actual = ruleCompositionService.addOutputParameterToRule(irodsRuleFileAsFile.getAbsolutePath(),
				newParamName);

		Assert.assertNotNull("null rule returned", actual);

		Assert.assertEquals("expected 1 new param", nbrBefore + 1, actual.getOutputParameters().size());

		boolean foundNew = false;

		for (int i = 0; i < actual.getOutputParameters().size(); i++) {
			if (actual.getOutputParameters().get(i).getUniqueName().equals(newParamName)) {
				foundNew = true;
				break;
			}
		}
	...
	public void testParseMsiAclPolicyRule() throws Exception {
		...
		Assert.assertFalse("empty rule body", rule.getRuleBody().isEmpty());
		Assert.assertTrue("not empty input parms", rule.getInputParameters().isEmpty());
		Assert.assertTrue("not empty output parms", rule.getOutputParameters().isEmpty());
	...
	public void testParseQueensRule() throws Exception {
		...
		Assert.assertFalse("empty rule body", rule.getRuleBody().isEmpty());
		Assert.assertTrue("expected empty input parms", rule.getInputParameters().isEmpty());
		Assert.assertFalse("empty output parms", rule.getOutputParameters().isEmpty());

		// round trip

		irodsRuleFileAsFile = accessObjectFactory.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(targetIrodsCollection + "/" + irodsRuleFile2);
		irodsRuleFileAsFile.deleteWithForceOption();
