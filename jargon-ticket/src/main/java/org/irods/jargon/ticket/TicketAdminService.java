...
public interface TicketAdminService {
	 ...
	 * @param file
	 *            {@link IRODSFile} file existing IRODS file or collection
	 * @param ticketId
	 *            {@code String} ticketID used to specify ticket key to be used for
	 *            this ticket
	 ...
	 *         delete. {@code false} means that the delete was not successful, due
	 ...
	 *             {@link JargonException}
	 */
	Ticket getTicketForSpecifiedTicketString(String ticketId) throws DataNotFoundException, JargonException;
	 ...
	 *             {@link JargonException}
	 */
	List<Ticket> listAllTicketsForDataObjects(int partialStartIndex) throws JargonException;
	 ...
	 *             {@link JargonException}
	 */
	List<Ticket> listAllTicketsForCollections(int partialStartIndex) throws JargonException;
	 ...
	 * @param usesLimit
	 *            {@code int} value 0 which specifies the uses limit for the
	 *            specified ticket
	 * @return {@code boolean} that will be {@code true} if the ticket was found to
	 *         modify. {@code false} means that the modify was not successful, due
	 *         to the ticket not being found. This can be ignored.
	 * @throws JargonException
	 *             {@link JargonException}
	 * 
	 ...
	 *            {@code int} value which specifies the file write limit for the
	 *            specified ticket
	 * @return {@code boolean} that will be {@code true} if the ticket was found to
	 *         modify. {@code false} means that the modify was not successful, due
	 *         to the ticket not being found. This can be ignored.
	 ...
	 *            {@code int} value which specifies the byte write limit for the
	 *            specified ticket
	 * @return {@code boolean} that will be {@code true} if the ticket was found to
	 *         modify. {@code false} means that the modify was not successful, due
	 *         to the ticket not being found. This can be ignored.
	 ...
	 *            {@code Date} java.util.Date set to a valid date/time used to
	 *            specify expiration time of ticket
	 * @return {@code boolean} that will be {@code true} if the ticket was found to
	 *         modify. {@code false} means that the modify was not successful, due
	 *         to the ticket not being found. This can be ignored.
	 * @throws JargonException
	 ...
	 *         EXTREME caution with this method - for a regular user (rodsuser),
	 *         this will delete tickets just for that user. However for a rodsadmin
	 ...
	 * @param partialStartIndex
	 *            {@code int} that can be set to 0 if starting at the beginning.
	 *            This specifies
	 * @return {@code List} of {@link Ticket} objects for the collection at the
	 *         given absolute path
	 * @throws JargonException
	 *             {@link JargonException}
	 */
	List<Ticket> listAllTicketsForGivenCollection(String irodsAbsolutePath, int partialStartIndex)
			throws JargonException;
	 ...
	 *             {@link JargonException}
	 */
	List<Ticket> listAllTicketsForGivenDataObject(String irodsAbsolutePath, int partialStartIndex)
			throws FileNotFoundException, JargonException;

	/**
	 * This is a 'meta' method that can manage the creation of iRODS tickets, and
	 * the simultaneous setting of the various limits. This convenience method
	 * removes the need to put this somewhat complicated sequence of code into
	 * applications.
	 * <p>
	 * It must be noted that iRODS protocol clients cannot frame this sort of
	 * operation in a transaction, so there is a very small chance that the operaton
	 ...
	 *            the ticket type (READ or WRITE), and the absolute path. The rest
	 *            of the data can either be left alone, or it can be specified, and
	 *            this will be handled by the update process. For example, if you
	 *            set a write byte limit in the provided ticket, a call will be made
	 *            to establish that value.
	 *            <p>
	 *            Note that the {@code ticketString} in the {@code Ticket} may
	 *            either be specified, or it may be left blank. If left blank, a
	 *            ticket id will be randomly generated. The {@code Ticket} object
	 ...
	 * compares this ticket, reflecting the desired values, and the current data.
	 * The delta between the two, for fields that may be updated, is used to call
	 * appropriate update methods.
	 * <p>
	 * Note that this is not transactional, so there is some small chance that not
	 * all changes will occur, this method will return a {@code Ticket} object that
	 * reflects the final state of the data in iRODS.
	 * <p>
	 ...
	 *             {@link JargonException}
	 */
	Ticket compareGivenTicketToActualAndUpdateAsNeeded(Ticket ticketWithDesiredData)
	 ...
	 *             {@link JargonException}
	 */
	Ticket createTicketFromTicketObjectAsAdminForGivenUser(Ticket ticket, String userName)
			throws DuplicateDataException, DataNotFoundException, JargonException;
}
