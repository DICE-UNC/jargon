...
public class AssertionHelper {
	...
	protected StringBuilder computeFullPathToLocalFile(final String filePathRelativeToScratch) {
	 ...
	 * system, recursively walk each tree and compare length, file name, and number
	 * of subdirectories/files.
	 *
	 * @param dir1
	...
	public void assertLocalDirectoriesHaveSameData(final String dir1, final String dir2)
		...
		if (file1Files.length != file2Files.length) {
			throw new IRODSTestAssertionException("mismatch of number of files in a directory, file1 has:"
					+ file1Files.length + " files, while file2 has:" + file2Files.length);
		}
	...
	/**
	 * Recursively match two files/directories for length, number of members, and
	 * name
	 *
	 * @param file1
	...
	public void assertTwoFilesAreEqualByRecursiveTreeComparison(final File file1, final File file2)
		...
		if (file1.isDirectory() && file2.isDirectory()) {
			...
			if (file1Files.length != file2Files.length) {
				throw new IRODSTestAssertionException("directories differ in the number of files contained, dir1 is: "
			...
			if (file1.getName().equals(file2.getName())) {
				...
				throw new IRODSTestAssertionException("file names are different, file1 abs path is:"
						+ file1.getAbsolutePath() + " while file2 abs path is:" + file2.getAbsolutePath());
			}

		} else {
			throw new IRODSTestAssertionException("file mismatch, one is a file, the other is a directory - file1:"
					+ file1.getAbsolutePath() + " file2:" + file2.getAbsolutePath());
		}
	...
	public void assertDataObjectFlaggedWithAVU(final String irodsAbsolutePath, final String avuAttribute,
		...
		try {
			...
			query.add(AVUQueryElement.instanceForValueQuery(AVUQueryElement.AVUQueryPart.ATTRIBUTE,
	...
	public void assertIrodsFileMatchesLocalFileChecksum(final String irodsAbsolutePath, final String localAbsolutePath,
			final IRODSAccessObjectFactory irodsAccessObjectFactory, final IRODSAccount irodsAccount)
