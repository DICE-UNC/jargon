 ...
 * iRODS data objects and collections as typical {@code java.io.*} objects that
 * can be found in the {@code org.irods.jargon.core.pub.io.*} package.
 * <p>
...
public final class DataTransferOperationsImpl extends IRODSGenericAO implements DataTransferOperations {
	...
	protected DataTransferOperationsImpl(final IRODSSession irodsSession, final IRODSAccount irodsAccount)
		...
		transferOperationsHelper = TransferOperationsHelper.instance(irodsSession, irodsAccount);
	 ...
	 */
	@Override
	public void physicalMove(final String absolutePathToSourceFile, final String targetResource)
			throws JargonException {
	...
	private void moveOperation(final IRODSFile irodsSourceFile, final IRODSFile irodsTargetFile)
		...
		if (irodsSourceFile.isFile() && irodsTargetFile.isDirectory()) {
			...
			actualTargetFile = getIRODSFileFactory().instanceIRODSFile(irodsTargetFile.getAbsolutePath(),
		...
		if (irodsSourceFile.isFile()) {
			...
			dataObjCopyInp = DataObjCopyInp.instanceForRenameFile(irodsSourceFile.getAbsolutePath(),
					actualTargetFile.getAbsolutePath());
		} else {

			log.info("move is for a collection");
			dataObjCopyInp = DataObjCopyInp.instanceForRenameCollection(irodsSourceFile.getAbsolutePath(),
					actualTargetFile.getAbsolutePath());
		}
	 ...
	 */
	@Override
	public void move(final String sourceFileAbsolutePath, final String targetFileAbsolutePath) throws JargonException {
	...
	public void move(final IRODSFile sourceFile, final IRODSFile targetFile)
		...
		if (sourceFile.getAbsolutePath().equals(targetFile.getAbsolutePath())) {
			...
			if (!targetFile.getResource().isEmpty()) {
				...
				physicalMove(sourceFile.getAbsolutePath(), targetFile.getResource());
				return;
			} else {
	 ...
	 */
	@Override
	public void getOperation(final IRODSFile irodsSourceFile, final File targetLocalFile,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock transferControlBlock) throws FileNotFoundException, JargonException {
		...
		log.info("get operation, irods source file is: {}", irodsSourceFile.getAbsolutePath());
		...
		try {
			...
			File targetLocalFileNameForCallbacks = new File(targetLocalFile.getAbsolutePath(),
					irodsSourceFile.getName());
			log.info("file name normalized:{}", targetLocalFileNameForCallbacks);

			/*
			 * See if I am rerouting connections, if so see if the file is on another
			 * resource
			 */
			log.info("am I rerouting?");
			if (operativeTransferControlBlock.getTransferOptions().isAllowPutGetResourceRedirects()
					&& getIRODSServerProperties().isSupportsConnectionRerouting()) {
			...
			if (reroutedAccount != null) {
				...
				reroutedDataTransferOperations.processGetAfterAnyConnectionRerouting(irodsSourceFile, targetLocalFile,
						transferStatusCallbackListener, operativeTransferControlBlock, targetLocalFileNameForCallbacks);

			} else {
				processGetAfterAnyConnectionRerouting(irodsSourceFile, targetLocalFile, transferStatusCallbackListener,
						operativeTransferControlBlock, targetLocalFileNameForCallbacks);
			}

		} catch (JargonException je) {
			log.warn("unexpected error in transfer that should have been caught in the actual transfer handling code",
					je);
			processExceptionDuringGetOperation(irodsSourceFile, targetLocalFile, transferStatusCallbackListener,
			...
			if (reroutedAccount != null) {
				...
				getIRODSAccessObjectFactory().closeSessionAndEatExceptions(reroutedAccount);
			}
		}
	}
	...
	private IRODSAccount checkForReroutedConnectionDuringGetOperation(final IRODSFile irodsSourceFile)
		...
		if (detectedHost == null || detectedHost.equals(FileCatalogObjectAOImpl.USE_THIS_ADDRESS)
			...
			reroutedAccount = IRODSAccount.instanceForReroutedHost(getIRODSAccount(), detectedHost);
		}
	...
	protected void processGetAfterAnyConnectionRerouting(final IRODSFile irodsSourceFile, final File targetLocalFile,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock operativeTransferControlBlock, final File targetLocalFileNameForCallbacks)
			throws FileNotFoundException, JargonException {
		...
		try {
			objStat = collectionAndDataObjectListAndSearchAO
					.retrieveObjectStatForPath(irodsSourceFile.getAbsolutePath());
		} catch (FileNotFoundException e) {
			log.error("file not found retrieving objStat for file:{}", irodsSourceFile.getAbsolutePath(), e);
			processExceptionDuringGetOperation(irodsSourceFile, targetLocalFileNameForCallbacks,
					transferStatusCallbackListener, operativeTransferControlBlock, e);
			return;
		}

		/*
		 * Compute the count of files to be transferred. This is different depending on
		 * whether this is a single file, or whether it's a collection.
		 */
		if (objStat.isSomeTypeOfCollection()) {
			...
			} else if (!targetLocalFileNameForCallbacks.getParentFile().canWrite()) {
			...
			/*
			 * Check for any cancellation after sending that initial status callback to
			...
			if (operativeTransferControlBlock.isCancelled()) {
				...
				if (transferStatusCallbackListener != null) {
					...
					TransferStatus status = TransferStatus.instance(TransferType.GET, irodsSourceFile.getAbsolutePath(),
							...
							operativeTransferControlBlock.getTotalFilesSkippedSoFar(),
							operativeTransferControlBlock.getTotalFilesTransferredSoFar(),
							operativeTransferControlBlock.getTotalFilesToTransfer(), TransferState.CANCELLED,
							getIRODSAccount().getHost(), getIRODSAccount().getZone());
					transferStatusCallbackListener.overallStatusCallback(status);
				}
			...
			getOperationWhenSourceFileIsDirectory(irodsSourceFile, targetLocalFile, transferStatusCallbackListener,
			...
			if (transferStatusCallbackListener != null) {
				TransferStatus status = TransferStatus.instance(TransferType.GET, irodsSourceFile.getAbsolutePath(),
						targetLocalFile.getAbsolutePath(), "", 0, 0, 0, 0,
						operativeTransferControlBlock.getTotalFilesToTransfer(), TransferState.OVERALL_INITIATION,
						getIRODSAccount().getHost(), getIRODSAccount().getZone());
			...
			transferOperationsHelper.processGetOfSingleFile(irodsSourceFile, targetLocalFile,
	 ...
	 */
	@Override
	public void getOperation(final String irodsSourceFileAbsolutePath, final String targetLocalFileAbsolutePath,
			final String sourceResourceName, final TransferStatusCallbackListener transferStatusCallbackListener,
	...
	private void processExceptionDuringGetOperation(final IRODSFile irodsSourceFile, final File targetLocalFile,
		...
		if (transferStatusCallbackListener != null) {
			...
			TransferStatus status = TransferStatus.instanceForException(TransferType.GET,
					irodsSourceFile.getAbsolutePath(), targetLocalFile.getAbsolutePath(), "", targetLocalFile.length(),
					targetLocalFile.length(), transferControlBlock.getTotalFilesTransferredSoFar(),
					transferControlBlock.getTotalFilesSkippedSoFar(), transferControlBlock.getTotalFilesToTransfer(),
					je, getIRODSAccount().getHost(), getIRODSAccount().getZone());
	...
	/**
	 * Initiate a recursive get operation. The iRODS source file is a collection,
	...
	private void getOperationWhenSourceFileIsDirectory(final IRODSFile irodsSourceFile, final File targetLocalFile,
		...
		log.info("this dir name, will be the new parent directory in the local file system:{}", thisDirName);

		File newParentDirectory = new File(targetLocalFile.getAbsolutePath(), thisDirName);

		boolean result = newParentDirectory.mkdir();
		if (!result) {
			log.warn("mkdirs for {} did not return success", newParentDirectory.getAbsolutePath());
		}

		log.debug("new parent directory created locally:{}", newParentDirectory.getAbsolutePath());

		transferOperationsHelper.recursivelyGet(irodsSourceFile, newParentDirectory, transferStatusCallbackListener,
				transferControlBlock);
	}
	 ...
	 */
	@Override
	public void putOperation(final File sourceFile, final IRODSFile targetIrodsFile,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock transferControlBlock) throws FileNotFoundException, JargonException {
		...
		try {
			...
			log.info("put operation for source: {}", sourceFile.getAbsolutePath());
			...
			if (targetIrodsFile.getResource().isEmpty()) {
				...
				targetIrodsFile.setResource(MiscIRODSUtils.getDefaultIRODSResourceFromAccountIfFileInZone(
						targetIrodsFile.getAbsolutePath(), getIRODSAccount()));
			}
			...
			if (operativeTransferControlBlock.getTransferOptions().isAllowPutGetResourceRedirects()
				...
				String detectedHost = dataObjectAO.getHostForPutOperation(targetIrodsFile.getAbsolutePath(),
			...
			/**
			 * If I rerouted, create a new transfer object for the new account, otherwise,
			 * proceed normally.
			 */
			if (reroutedAccount != null) {
				...
				DataTransferOperationsImpl reroutedDataTransferOperations = (DataTransferOperationsImpl) getIRODSAccessObjectFactory()
						.getDataTransferOperations(reroutedAccount);
				reroutedDataTransferOperations.processPutAfterAnyConnectionRerouting(sourceFile, targetIrodsFile,
						transferStatusCallbackListener, operativeTransferControlBlock);
			} else {
				log.info("process put with no rerouting");
				processPutAfterAnyConnectionRerouting(sourceFile, targetIrodsFile, transferStatusCallbackListener,
						operativeTransferControlBlock);
			}

		} catch (JargonException je) {
			log.warn("unexpected exception in put operation that should have been caught in the transfer handler", je);
			processExceptionDuringPutOperation(sourceFile, targetIrodsFile, transferStatusCallbackListener,
					operativeTransferControlBlock, je);
		} catch (Throwable e) {
			log.warn(
					"unexepected exception occurred in transfer, not caught in transfer code, will wrap in a JargonException and process",
	 ...
	 * @param transferStatusCallbackListener
	 *            {@link TransferStatusCallbackListener} with the optional listener
	 *            for progress information. May be {@code null}
	 * @param operativeTransferControlBlock
	...
	protected void processPutAfterAnyConnectionRerouting(final File sourceFile, final IRODSFile targetIrodsFile,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock operativeTransferControlBlock) throws JargonException {
		/*
		 * If a callback listener is given, make sure there is a transfer control block,
		 * and do a pre-scan of the iRODS collection to get a count of files to be
		 * transferred
		 */

		if (sourceFile.isDirectory()) {

			preCountLocalFilesBeforeTransfer(sourceFile, operativeTransferControlBlock);

			putWhenSourceFileIsDirectory(sourceFile, targetIrodsFile, transferStatusCallbackListener,
					operativeTransferControlBlock);
			...
			if (transferStatusCallbackListener != null) {
				TransferStatus status = TransferStatus.instance(TransferType.PUT, sourceFile.getAbsolutePath(),
						callbackTargetIrodsPath, "", operativeTransferControlBlock.getTotalBytesToTransfer(), 0L,
						operativeTransferControlBlock.getTotalFilesTransferredSoFar(),
						operativeTransferControlBlock.getTotalFilesSkippedSoFar(),
						operativeTransferControlBlock.getTotalFilesToTransfer(), TransferState.OVERALL_INITIATION,
						getIRODSAccount().getHost(), getIRODSAccount().getZone());
			...
			transferOperationsHelper.processPutOfSingleFile(sourceFile, targetIrodsFile, transferStatusCallbackListener,
					operativeTransferControlBlock);
	 ...
	 */
	@Override
	public void putOperation(final String sourceFileAbsolutePath, final String targetIrodsFileAbsolutePath,
			String targetResourceName, final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock transferControlBlock) throws JargonException {
		...
		IRODSFile targetFile = getIRODSFileFactory().instanceIRODSFile(targetIrodsFileAbsolutePath);
	...
	/**
	 * An exception has occurred in a put operation. This method will check for the
	 * existence of a callback listener for status. If one is supplied, the
	 * exception is reported to the callback listener and quashed. If no callback
	 ...
	 * @param transferStatusCallbackListener
	 *            {@link org.irods.jargon.core.transfer.TransferStatusCallbackListener}
	 *          implementation that will receive callbacks of
	 ...
	 *            set to {@code null} if those facilities are not needed.  This may be set to {@code null}, in
	 *            which case, exceptions that are thrown will be rethrown by this
	 *            method to the caller.
	 * @param transferControlBlock
	 *            {@link org.irods.jargon.core.transfer.TransferControlBlock}
	...
	private void processExceptionDuringPutOperation(final File sourceFile, final IRODSFile targetIrodsFile,
		...
		if (transferStatusCallbackListener != null) {
			...
			TransferStatus status = TransferStatus.instanceForException(TransferType.PUT, sourceFile.getAbsolutePath(),
	...
	/**
	 * A put operation has been initiated for a directory. This means that Jargon
	 * will recursively process the put operation. The containing folder of the
	 * source will be used as the new subdirectory in iRODS under which the data
	 * will be moved.
	 *
	 * @param sourceFile
	 *            {@code File} that is the source of the transfer.
	 * @param targetIrodsFile
	 *            {@link org.irods.jargon.core.pub.io.IRODSFile} that is the target
	 *            of the transfer.
	 * @param transferStatusCallbackListener
	 *            {@link org.irods.jargon.core.transfer.TransferStatusCallbackListener}
	 ...
	 *            success/failure of each individual file transfer. This may be set
	 *            to {@code null}, in which case, exceptions that are thrown will be
	 *            re-thrown by this method to the caller. 
	 * @param transferControlBlock
	 *            {@link org.irods.jargon.core.transfer.TransferControlBlock}
	 *            implementation that is the communications mechanism between the
	...
	private void putWhenSourceFileIsDirectory(final File sourceFile, final IRODSFile targetIrodsFile,
		...
		/*
		 * take the last path of the source, and make this a directory under iRODS.
		 * Then, the files will begin transfer under this newly created subdirectory
		 */
		...
		log.info("this dir name, will be the new parent directory in iRODS:{}", thisDirName);

		IRODSFile newIrodsParentDirectory = getIRODSFileFactory().instanceIRODSFile(targetIrodsFile.getAbsolutePath(),
				thisDirName);
		...
		if (transferStatusCallbackListener != null) {
			TransferStatus status = TransferStatus.instance(TransferType.PUT, sourceFile.getAbsolutePath(),
					newIrodsParentDirectory.getAbsolutePath(), "", transferControlBlock.getTotalBytesToTransfer(),
					transferControlBlock.getTotalBytesTransferredSoFar(),
					transferControlBlock.getTotalFilesTransferredSoFar(),
					transferControlBlock.getTotalFilesSkippedSoFar(), transferControlBlock.getTotalFilesToTransfer(),
					TransferState.OVERALL_INITIATION, getIRODSAccount().getHost(), getIRODSAccount().getZone());
			transferStatusCallbackListener.overallStatusCallback(status);
		}

		/**
		 * Check for any cancellation that might occur after the initial start of
		 * operation callback
		 */
		...
		if (transferControlBlock.isCancelled()) {
			...
			if (transferStatusCallbackListener != null) {
				...
				TransferStatus status = TransferStatus.instance(TransferType.PUT, sourceFile.getAbsolutePath(),
						...
						transferControlBlock.getTotalFilesTransferredSoFar(),
						transferControlBlock.getTotalFilesToTransfer(), TransferState.CANCELLED,
						getIRODSAccount().getHost(), getIRODSAccount().getZone());
				transferStatusCallbackListener.overallStatusCallback(status);
			}
		...
		try {
			...
			log.error("exeption in mkdir of: {}", newIrodsParentDirectory.getAbsolutePath());
			throw new JargonException(e);
		}

		transferOperationsHelper.recursivelyPut(sourceFile, newIrodsParentDirectory, transferStatusCallbackListener,
				transferControlBlock);

		/**
		 * Send an overall status callback. If the state is cancelled, see if it was
		 * cancelled due to too many errors, in which case it is called a failure
		 */
		if (transferStatusCallbackListener != null) {
			if (transferControlBlock.isCancelled() || transferControlBlock.isPaused()) {
				...
				TransferStatus status = TransferStatus.instance(TransferType.PUT, sourceFile.getAbsolutePath(),
						...
						transferControlBlock.getTotalFilesTransferredSoFar(),
						transferControlBlock.getTotalFilesSkippedSoFar(),
						transferControlBlock.getTotalFilesToTransfer(), transferState, getIRODSAccount().getHost(),
				...
				TransferStatus status = TransferStatus.instance(TransferType.PUT, sourceFile.getAbsolutePath(),
						...
						transferControlBlock.getTotalFilesTransferredSoFar(),
						transferControlBlock.getTotalFilesSkippedSoFar(),
						transferControlBlock.getTotalFilesToTransfer(), TransferState.OVERALL_COMPLETION,
						getIRODSAccount().getHost(), getIRODSAccount().getZone());
				transferStatusCallbackListener.overallStatusCallback(status);
			}
		}
	 ...
	 */
	@Override
	public void replicate(final String irodsFileAbsolutePath, final String targetResource,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock transferControlBlock) throws JargonException {
		...
		if (sourceFile.isDirectory()) {
			...
			preCountIrodsFilesBeforeTransfer(irodsFileAbsolutePath, operativeTransferControlBlock);

			// send 0th file status callback that indicates startup
			if (transferStatusCallbackListener != null) {
				TransferStatus status = TransferStatus.instance(TransferType.REPLICATE, sourceFile.getAbsolutePath(),
			...
			try {
				transferOperationsHelper.recursivelyReplicate(sourceFile, targetResource,
						transferStatusCallbackListener, operativeTransferControlBlock);
				...
				if (transferStatusCallbackListener != null) {
					...
					TransferStatus status = TransferStatus.instanceForException(TransferType.REPLICATE,
							...
							operativeTransferControlBlock.getTotalFilesTransferredSoFar(),
							operativeTransferControlBlock.getTotalFilesSkippedSoFar(),
							operativeTransferControlBlock.getTotalFilesToTransfer(), e, getIRODSAccount().getHost(),
							getIRODSAccount().getZone());
			...
			processReplicationOfSingleFile(irodsFileAbsolutePath, targetResource, transferStatusCallbackListener,
					operativeTransferControlBlock);
	...
	public void copy(final String irodsSourceFileAbsolutePath, final String targetResource,
			...
			final TransferStatusCallbackListener transferStatusCallbackListener, final boolean force,
			final TransferControlBlock transferControlBlock)
		...
		IRODSFile targetFile = getIRODSFileFactory().instanceIRODSFile(irodsTargetFileAbsolutePath);

		// look for recursive copy (collection to collection) and process,
		// otherwise, just copy the file

		if (sourceFile.isDirectory()) {
			processCopyWhenSourceIsDir(targetResource, transferStatusCallbackListener, operativeTransferControlBlock,
					sourceFile, targetFile);

		} else {
			processCopyWhenSourceIsAFile(targetResource, transferStatusCallbackListener, operativeTransferControlBlock,
					sourceFile, targetFile);
		}
	}
	...
	public void copy(final String irodsSourceFileAbsolutePath, final String targetResource,
		...
		copy(sourceFile, targetFile, transferStatusCallbackListener, transferControlBlock);
	}
	 ...
	 */
	@Override
	public void copy(final IRODSFile irodsSourceFile, final IRODSFile irodsTargetFile,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock transferControlBlock)
		...
		if (irodsTargetFile.getResource().isEmpty()) {
			...
			irodsTargetFile.setResource(getIRODSAccount().getDefaultStorageResource());
		}
		...
		if (irodsSourceFile.isDirectory()) {
			processCopyWhenSourceIsDir(irodsTargetFile.getResource(), transferStatusCallbackListener,
					operativeTransferControlBlock, irodsSourceFile, irodsTargetFile);

		} else {
			processCopyWhenSourceIsAFile(irodsTargetFile.getResource(), transferStatusCallbackListener,
					operativeTransferControlBlock, irodsSourceFile, irodsTargetFile);
		}
	}

	private void processCopyWhenSourceIsAFile(final String targetResource,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock operativeTransferControlBlock, final IRODSFile sourceFile, IRODSFile targetFile)
			throws OverwriteException, FileNotFoundException, JargonException {
		...
		// send 0th file status callback that indicates startup
		if (transferStatusCallbackListener != null) {
			TransferStatus status = TransferStatus.instance(TransferType.COPY, sourceFile.getAbsolutePath(),
		...
		transferOperationsHelper.processCopyOfSingleFile(sourceFile.getAbsolutePath(), targetResource,
				targetFile.getAbsolutePath(), transferStatusCallbackListener, operativeTransferControlBlock);
	...
	private void processCopyWhenSourceIsDir(final String targetResource,
			final TransferStatusCallbackListener transferStatusCallbackListener,
			final TransferControlBlock operativeTransferControlBlock, final IRODSFile sourceFile, IRODSFile targetFile)
			throws OverwriteException, FileNotFoundException, JargonException {
		log.info("this copy operation is recursive");

		preCountIrodsFilesBeforeTransfer(sourceFile.getAbsolutePath(), operativeTransferControlBlock);
		...
		if (targetFile.exists() && targetFile.isFile()) {
			...
			log.error("attempt to copy a directory to a file: {}", targetFile.getAbsolutePath());
		...
		transferOperationsHelper.recursivelyCopy(sourceFile, targetResource, targetFile.getAbsolutePath(),
				transferStatusCallbackListener, operativeTransferControlBlock);
	 ...
	 * @throws JargonException
	 */
	private void preCountIrodsFilesBeforeTransfer(final String irodsFileAbsolutePath,
			final TransferControlBlock operativeTransferControlBlock) throws FileNotFoundException, JargonException {
	...
	private void processReplicationOfSingleFile(final String irodsFileAbsolutePath, final String targetResource,
		...
		transferOperationsHelper.processReplicationOfSingleFile(irodsFileAbsolutePath, targetResource,
	 ...
	 */
	public void setTransferOperationsHelper(final TransferOperationsHelper transferOperationsHelper) {
