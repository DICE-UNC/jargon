 ...
 * so that synch overhead is minimized.  Note that the worst case side effect would be asking
 * iRODS for something (e.g. trying to run a specific query) more than once, so I'm trying to 
 * minimize synchronization.  We can allow some 'fuzziness' here.  The point is to minimize such
 * redundant calls.
 * 
...
public class DiscoveredServerPropertiesCache {
	...
	private ConcurrentHashMap<String,ConcurrentHashMap<String, String>> discoveredServerPropertiesCache = new ConcurrentHashMap<String, ConcurrentHashMap<String, String>>(8, 0.9f, 1);
	...
	/**
	 * Get the value cached for this host and zone.  Note that if the zone does not exist in the cache, or
	 * if the property does not exist in the zone cache, a <code>null</code> will be returned.
	 * 
	 * @param host <code>String</code> with the name of the iRODS host this applies to
	 * @param zoneName <code>String</code> with the name of the iRODS zone this applies to
	 * @param propertyName <code>String</code> with the property value to be retrieved
	 * @return <code>String</code> with the value for the given property, or <code>null</code> if no such
	 * value exists
	 */
	...
	/**
	 * Delete the property from the cache if it exists.  If the zone has no cache, or the property 
	 * itself is not cached, silently ignore
	 * @param host
	 * @param zoneName
	 * @param propertyName
	 */
	 ...
	 * @param host <code>String</code> with the name of the iRODS host this applies to
	 * @param zoneName <code>String</code> with the name of the iRODS zone this applies to
	 * @param propertyName <code>String</code> with the property to set
	 * @param value <code>String</code> with a non-null value to set. For consistency, set to blank if a property has no value.
	 */
	public void cacheAProperty(final String host, final String zoneName, final String propertyName, final String value) {
		...
		getCacheForHostAndZoneAndAddIfNotThere(host, zoneName).put(propertyName, value);
		
	}
	
	/**
	 * Look for a properties map for the given host and zone, add to the main cache if it doesn't exist
	 * @param host
	 * @param zoneName
	 * @return
	 */
	...
	/**
	 * Standard way to concatenate the host and zone name, trimming white space in the process. This allows consistent look up
	 * @param host
	 * @param zoneName
	 * @return
	 */
