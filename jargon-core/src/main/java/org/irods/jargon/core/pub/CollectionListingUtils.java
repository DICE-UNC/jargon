...
class CollectionListingUtils {
	 ...
	 */
	 List<CollectionAndDataObjectListingEntry> handleNoListingUnderRootOrHomeByLookingForPublicAndHome(
		...
		sb.append("/");
		sb.append(collectionAndDataObjectListAndSearchAO.getIRODSAccount().getZone());
		...
		if (collectionAndDataObjectListAndSearchAO.getIRODSAccount().isAnonymousAccount()) {
	 ...
	 private CollectionAndDataObjectListingEntry createStandInForZoneDir() {
		...
		private CollectionAndDataObjectListingEntry createStandInForUserDir(
			...
			sb.append("/home");
			entry.setParentPath(sb.toString());
			sb.append(collectionAndDataObjectListAndSearchAO.getIRODSAccount().getUserName());
			entry.setPathOrName(sb.toString());
			entry.setSpecColType(objStat.getSpecColType());
			entry.setCreatedAt(objStat.getCreatedAt());
			entry.setId(objStat.getDataId());
		 ...
		 */
		 ...
		 List<CollectionAndDataObjectListingEntry> listDataObjectsUnderPathWhenSpecColl(
				final ObjStat objStat, final int partialStartIndex,
		...
		private List<CollectionAndDataObjectListingEntry> listCollectionsUnderPathViaGenQuery(
		 ...
		 IRODSQueryResultSet queryForPathAndReturnResultSet(
			...
			IRODSGenQueryExecutor irodsGenQueryExecutor = collectionAndDataObjectListAndSearchAO.getIRODSAccessObjectFactory().getIRODSGenQueryExecutor(collectionAndDataObjectListAndSearchAO.getIRODSAccount());
			...
			try {
				irodsQuery = builder
						.exportIRODSQueryFromBuilder(collectionAndDataObjectListAndSearchAO.getJargonProperties()
		...
		/**
		 * List the data objects underneath the given path given an already obtained <code>ObjStat</code>
		 * @param objStat
		 * @param partialStartIndex
		 * @return
		 * @throws JargonException
		 */
		 List<CollectionAndDataObjectListingEntry> listDataObjectsUnderPath(
			...
			if (objStat.getSpecColType() == SpecColType.STRUCT_FILE_COLL
				...
				files = listDataObjectsUnderPathWhenSpecColl(objStat, partialStartIndex,
						effectiveAbsolutePath);
			} else {
		 ...
		 *            confusing otherwise.
		 */
		 void augmentCollectionEntryForSpecialCollections(
