...
class CollectionListingUtils {
    ...
    List<CollectionAndDataObjectListingEntry> handleNoListingUnderRootOrHomeByLookingForPublicAndHome(
        ...
        if (path.equals(comparePath)) {
            ...
            sb.append("/");
            sb.append(collectionAndDataObjectListAndSearchAO
     ...
     *
     * @param objStat {@link ObjStat} from iRODS that details the nature of the
     * collection
     ...
     *
     * @param objStat {@link ObjStat} retreived for the parent directory
     * @param effectiveAbsolutePath <code>String</code> with the path used to
     * query, this will be the canonical path for the parent collection, and
     * should correspond to the absolute path information in the given
     * <code>entry</code>.
     * @param entry {@link CollectionAndDataObjectListingEntry} which is the raw
     * data returned from querying the iCat based on the
     * <code>effectiveAbsolutePath</code>. This information is from the
     * perspective of the canonical path, and the given method will reframe the
     * <code>entry</code> from the perspective of the requested path This means
     * that a query on children of a soft link carry the data from the
     * perspective of the soft linked directory, even though the iCAT carries
     * the information based on the 'source path' of the soft link. This gets
