...
public class IRODSRandomAccessFile implements DataInput, DataOutput {
	 ...
	 * each value in a boolean array to true if the corresponding byte is nonzero,
	 * false if that byte is zero. This method is suitable for reading the byte
	...
	/**
	 * Reads up to {@code b.length} bytes of data from this file into an array of
	...
	/**
	 * Reads a Unicode character from this file. This method reads two bytes from
	 * the file, starting at the current file pointer. If the bytes read, in order,
	 * are {@code b1} and {@code b2}, where
	 * {@code 0&nbsp;&lt;=&nbsp;b1,&nbsp;b2&nbsp;&lt;=&nbsp;255}, then the result is
	 * equal to: <blockquote>
	 *
	 * <pre>
	 * (char) ((b1 &lt;&lt; 8) | b2)
	 ...
	 * <P>
	 * The data read by this method is converted from the file's binary data format
	 * to that of host running this application. This may involve a change in byte
	 * order and data type size.
	 * <P>
	...
	/**
	 * Reads {@code b.length} bytes from this file into the byte array, starting at
	 * the current file pointer. This method reads repeatedly from the file until
	...
	/**
	 * Reads exactly {@code len} bytes from this file into the byte array, starting
	 * at the current file pointer. This method reads repeatedly from the file until
	 * the requested number of bytes are read. This method blocks until the
	 * requested number of bytes are read, the end of the stream is detected, or an
	 * exception is thrown.
	 *
	...
	/**
	 * Reads input bytes and returns an int value. The number of bytes read is equal
	 * to the size of an int using the current binary data format.
	 * <P>
	 * The data read by this method is converted from the file's binary data format
	 * to that of host running this application. This may involve a change in byte
	 * order and data type size.
	 * <P>
	 * This method is suitable for reading the bytes written by the writeInt method
	 ...
	 * {@code '&#92;r'}), a newline character ({@code '&#92;n'}), a carriage-return
	 * character immediately followed by a newline character, or the end of the
	 * file. Line-terminating characters are discarded and are not included as part
	...
	/**
	 * Reads input bytes and returns a long value. The number of bytes read is equal
	 * to the size of a long using the current binary data format.
	 * <P>
	 * The data read by this method is converted from the file's binary data format
	 * to that of host running this application. This may involve a change in byte
	 * order and data type size.
	 * <P>
	 * This method is suitable for reading the bytes written by the writeLong method
	 * of interface BinaryDataOutput.
	 * <P>
	 *
	 * @return the value read
	 ...
	 * by {@code readUnsignedShort}. This value gives the number of following bytes
	 * that are in the encoded string, not the length of the resulting string. The
	 * following bytes are then interpreted as bytes encoding characters in the
	...
	/**
	 * Reads input bytes and returns an unsigned value. The number of bytes read is
	 * equal to the size of an unsigned short using the current binary data format.
	 * <P>
	 * The data read by this method is converted from the file's binary data format
	 * to that of host running this application. This may involve a change in byte
	 * order and data type size.
	 ...
	 * {@code n} bytes have been skipped is only one possibility. This method never
	 ...
	 * argument. The number of bytes written is equal to the size of a char using
	 * the current binary data format.
	 * <P>
	 * The data written by this method is converted from the host's binary data
	 * format to that for the file. This may involve a change in byte order and data
	 * type size.
	 * <P>
	 ...
	 * argument. The number of bytes written is equal to the size of a double using
	 * the current binary data format.
	 * <P>
	 * The data written by this method is converted from the host's binary data
	 * format to that for the file. This may involve a change in byte order and data
	 ...
	 * argument. The number of bytes written is equal to the size of a float using
	 * the current binary data format.
	 * <P>
	 * The data written by this method is converted from the host's binary data
	 * format to that for the file. This may involve a change in byte order and data
	 ...
	 * The number of bytes written is equal to the size of an int using the current
	 * binary data format.
	 * <P>
	 * The data written by this method is converted from the host's binary data
	 * format to that for the file. This may involve a change in byte order and data
	 ...
	 * argument. The number of bytes written is equal to the size of a long using
	 * the current binary data format.
	 * <P>
	 * The data written by this method is converted from the host's binary data
	 * format to that for the file. This may involve a change in byte order and data
	 ...
	 * argument. The number of bytes written is equal to the size of a short using
	 * the current binary data format.
	 * <P>
	 * The data written by this method is converted from the host's binary data
	 * format to that for the file. This may involve a change in byte order and data
	 ...
	 * pointer, as if by the {@code writeShort} method giving the number of bytes to
	 ...
	 * the range 0 to 255 ({@code 0x00-0x0ff}). This method blocks if no input is
	 * yet available.
	 * <p>
	...
	public int read() throws IOException {
		...
		int read;
		try {
			read = fileIOOperations.fileRead(irodsFile.getFileDescriptor(), buffer, offset, 1);
	...
	protected int readBytes(final byte buffer[], final int offset, final int len) throws IOException {
		int read;
		try {
			read = fileIOOperations.fileRead(irodsFile.getFileDescriptor(), buffer, offset, len);
		} catch (JargonException e) {
			log.error("JargonException reading file", e);
			throw new IOException(e);
		}
	...
	protected void writeBytes(final byte buffer[], final int offset, final int len) throws IOException {
		try {
			filePointer += fileIOOperations.write(irodsFile.getFileDescriptor(), buffer, offset, len);
		} catch (JargonException e) {
			log.error("JargonException reading file", e);
			throw new IOException(e);
		}
	 ...
	 * offset may be set beyond the end of the file. Setting the offset beyond the
	...
	public void seek(final long position, final SeekWhenceType origin) throws IOException {
		...
		try {
			fileIOOperations.seek(irodsFile.getFileDescriptor(), position, origin);
		} catch (JargonException e) {
			log.error("JargonException reading file", e);
			throw new IOException(e);
		}
	...
	/**
	 * Reads a {@code boolean} from this file. This method reads a single byte from
	 * the file, starting at the current file pointer. A value of {@code 0}
	 * represents {@code false}. Any other value represents {@code true}. This
	 * method blocks until the byte is read, the end of the stream is detected, or
	 ...
	 * {@code b}, where {@code 0&nbsp;&lt;=&nbsp;b&nbsp;&lt;=&nbsp;255}, then the
	 * result is: <blockquote>
	 *
	 * <pre>
	 * (byte) (b)
	 * </pre>
