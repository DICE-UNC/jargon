...
public class GeneralAdminInpForSQ extends
	...
	public Tag getTagValue() throws JargonException {
		...
		// need to shuffle things a bit for the specific query remove function which
		// allows either an sql query or alias name as arg2 and it does not use arg3 at all
		// the server api code decides whether arg2 is a query or alias
		// if it is a query it will remove all matching queries whether or not a unique alias
		// is supplied!
		if (this.action == SQ_RM) {
			// check to see if this is remove by alias - if so put alias name in arg2
			if (arg2 == BLANK) {
				arg2 = arg3;
				arg3 = BLANK;
			}
		...
		Tag message = new Tag(PI_TAG, new Tag[] { 
				new Tag(ARG0, getAction()),
				new Tag(ARG1, ADMIN_OBJ),
				new Tag(ARG2, arg2),
				new Tag(ARG3, arg3),
				new Tag(ARG4, BLANK),
				new Tag(ARG5, BLANK),
