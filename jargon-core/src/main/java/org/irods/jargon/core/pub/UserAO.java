...
public interface UserAO extends IRODSAccessObject {
	 ...
	 *
	 * @param user
	 *            {@link org.irods.jargon.core.pub.domain.User} with information on
	...
	/**
	 * Query users and return the {@code User} object with the given user name. Note
	 * that user names may be given in user#zone format, and that federated user
	 * registered on the current zone will be returned.
	 * <p>
	 * For example, if I have zone1 and zone2, and zone1 has registered user1#zone2
	 * as a user in zone1, then this method will get the information that zone1 has
	 * on the user name user#zone2.
	 * <p>
	 * This is distinct from going to zone2, and asking for information on the user
	 * user1#zone2.
	 *
	 * @param name
	...
	/**
	 * Query users by the unique id assigned by iRODS (database unique key). This
	 * will default to searching the current zone
	 *
	 * @param userId
	 ...
	 */
	User findById(final String userId) throws JargonException, DataNotFoundException;

	/**
	 * Query users by the unique id assigned by iRODS (database unique key) in a
	 * zone.
	 * <p>
	 * This will, if the given zone is not the same as the current zone, initiate a
	 * cross-zone query and retrieve the information from the given zone name.
	 *
	 * @param userId
	 *            {@code String} with the unique database key for the user.
	 * @return {@link org.irods.jargon.core.pub.domain.User}
	 ...
	 *             if the user does not exist
	 */
	User findByIdInZone(String userId, String zone) throws JargonException, DataNotFoundException;
	 ...
	 * @param userId
	 *            {@code String} with the unique database key for the user.
	 * @return {@code List} of {@link org.irods.jargon.core.pub.domain.AvuData} with
	 *         query results.
	 * @throws JargonException
	 */
	 ...
	 *            {@code String} containing iquest syntax for a where statement,
	 ...
	 * changing their own password. Jargon will use an obfuscation routine to send
	 * the new password to iRODS.
	 *
	 * @param userName
	...
	/**
	 * Change the password for a given user. This method is used by an admin setting
	 * the password for an arbitrary user. For a user changing their own password,
	 * use the {@code changeAPasswordByThatUser(String, String)} method
	 *
	 * @param userName
	...
	/**
	 * Add the AVU metadata for the given user. This is only possible when a rods
	 * admin.
	 *
	 * @param userName
	 *            {@code String} with the user name to whom the AVU metadata will be
	 *            added
	 * @param avuData
	...
	/**
	 * * Add or update the AVU metadata for the given user. This is only possible
	 * when a rods admin. This method is only available on iRODS 4.1+ and is more
	 * straightforward than the add or modify.
	 * <p/>
	 ...
	 *
	 * @param userName
	 *            {@code String} with the user name from whom the AVU metadata will
	 *            be removed
	 * @param avuData
	...
	/**
	 * Generate a temporary password for another user. Password validity times and
	 * number of connections will be set by the iRODS server.
	 * <p>
	 * This is a rodsadmin only function, and was added post iRODS 3.0.
	 *
	 * @param targetUserName
	 *            {@code String} (required) with the user name for which the
	...
	/**
	 * Given a unique numeric user ID, retrieve the user's distinguished name. Note
	 * that the various list methods do not retrieve the DN by default, as it causes
	 * unnecessary GenQueries to be issued per user. This method can retrieve that
	 * data as needed.
	 * <p>
	 ...
	 *            {@code String} with the name of the user
	 * @param userInfo
	 *            {@code String} with the info to set, it can be blank, but not
	 *            {@code null}
	 * @throws DataNotFoundException
	...
	/**
	 * Remove the DN for the given user. If the user or DN does not exist, it will
