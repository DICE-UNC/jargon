...
public interface RuleProcessingAO extends IRODSAccessObject {
	 ...
	 *
	 *
	 * @param irodsRuleAsString {@code String} containing an iRODS rule as would be
	 *                          submitted via irule
	 * @return {@link IRODSRuleExecResult} with the rule response
	 * @throws JargonRuleException error in translation of the rule, typically
	 *                             syntax
	 * @throws JargonException     other error in irods or jargon
	 */
	 ...
	 *
	 * @param resourcePath             {@code String} that will be a path from which
	 *                                 the resource will be loaded, using the rules
	 *                                 associated with
	 *                                 {@code Class.getResourceAsStream(String)}.
	 * @param irodsRuleInputParameters {@code List} of {@link IRODSRuleParameter}
	 *                                 with overrides to parameters defined in the
	 *                                 rule file. This can be set to {@code null} if
	 *                                 no overrides are needed.
	 * @param ruleProcessingType       {@link RuleProcessingAO.RuleProcessingType}
	 *                                 that describes how parameters are resolved
	 *                                 (@internal, @external, classic for rules in
	 *                                 the classic rule language, pre iRODS 3.0).
	 * @return {@link IRODSRuleExecResult}
	 * @throws DataNotFoundException if rule missing
	 * @throws JargonException       for iRODS error
	 */
	 ...
	 * overridden by any values passed in as an input parameter.
	 *
	 * @param resourcePath                {@code String} that will be a path from
	 *                                    which the resource will be loaded, using
	 *                                    the rules associated with
	 *                                    {@code Class.getResourceAsStream(String)}.
	 * @param irodsRuleInputParameters    {@code List} of {@link IRODSRuleParameter}
	 *                                    with overrides to parameters defined in
	 *                                    the rule file. This can be set to
	 *                                    {@code null} if no overrides are needed.
	 * @param ruleInvocationConfiguration {@link RuleInvocationConfiguration} that
	 *                                    describes the rule type and processing
	 *                                    parameters, including the rule engine
	 *                                    instance it relates to
	 * @return {@link IRODSRuleExecResult}
	 * @throws DataNotFoundException if rule missing
	 * @throws JargonException       for iRODS error
	 */
	 ...
	 * overridden by any values passed in as an input parameter.
	 *
	 * @param ruleFileAbsolutePath        {@code String} with the absolute path to a
	 *                                    file in iRODS containing the rule
	 * @param irodsRuleInputParameters    {@code List} of {@link IRODSRuleParameter}
	 *                                    with overrides to parameters defined in
	 *                                    the rule file. This can be set to
	 *                                    {@code null} if no overrides are needed.
	 ...
	 * string "10".
	 *
	 * @param irodsRuleAsString       {@code String} with the rule body, as well as
	 *                                input and output parameters.
	 * @param inputParameterOverrides {@code List} of {@link IRODSRuleParameter}
	 *                                that overrides the parameters derived from the
	 *                                rule bodies.
	 * @param ruleProcessingType      {@link RuleProcessingAO.RuleProcessingType}
	 *                                enum value. Note that it should be set to
	 *                                {@code CLASSIC} for classic rules, and
	 *                                {@code EXTERNAL} or {@code INTERNAL} for new
	 *                                format rules.
	 ...
	 *
	 * @param irodsRuleAsString           {@code String} with the rule body, as well
	 *                                    as input and output parameters.
	 * @param inputParameterOverrides     {@code List} of {@link IRODSRuleParameter}
	 *                                    that overrides the parameters derived from
	 *                                    the rule bodies.
	 *
	 * @param ruleInvocationConfiguration {@link RuleInvocationConfiguration} that
	 *                                    represents the type of rule engine the
	 *                                    user wishes this rule to be run on. With
	 *                                    multiple rule engines iRODS needs this
	 *                                    information to properly process the rule.
	 *                                    If set to AUTO_DETECT Jargon will try and
	 *                                    guess (but may not be too smart at first).
	 * @return {@link IRODSRuleExecResult} that represents the results of processing
	 *         the rule.
	 * @throws JargonRuleException if an exception occurred in rule translation.
	 * @throws JargonException     if iRODS processing resulted in an error.
	 ...
	 *
	 * @param queueId {@code int} with an id that relates to a stored rule to be
	 *                removed
	 * @throws JargonException for iRODS error
	 */
	...
	/**
	 * Execute a rule provided as a string following the 'new' format of JSON-ized
	 * input params and a separate set of config parameters. <b>WARNING</b> this
	 * should currently be considered experimental. iRODS may have some issues
	 * processing JSON strings as input parameters. FIXME: evaluate level of iRODS
	 * support before release and update this comment
	 * 
	 * @param ruleString                  {@code String} with the rule text
	 * @param irodsRuleInputParameters    {@code List} of {@link IRODSRuleParameter}
	 *                                    for the input parameters
	 * @param irodsRuleContextParameters  {@code List} of {@link IRODSRuleParameter}
	 *                                    for the config parameters
	 * @param ruleInvocationConfiguration {@link RuleInvocationConfiguration}
	 * @return {@link IRODSRuleExecResult} that represents the results of processing
	 *         the rule.
	 * @throws JargonRuleException if an exception occurred in rule translation.
	 * @throws JargonException     if iRODS processing resulted in an error.
	 */
	IRODSRuleExecResult executeJsonStyleRule(final String ruleString,
