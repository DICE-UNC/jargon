...
public class QuotaAOImpl extends IRODSGenericAO implements QuotaAO {
	 ...
	 */
	protected QuotaAOImpl(final IRODSSession irodsSession,
	 ...
	 */
	@Override
	public void setUserQuotaTotal(final String userName, final long quotaValue)
			throws JargonException {
		...
		GeneralAdminInp adminPI = GeneralAdminInp.instanceForSetUserQuotaTotal(
				userName, quotaValue);
	 ...
	 */
	@Override
	public List<Quota> listQuotaForAUser(final String userName)
			throws JargonException {
	 ...
	 */
	@Override
	public Quota getGlobalQuotaForAUser(final String userName)
			throws JargonException {
	...
	private SimpleQueryExecutorAO getSimpleQueryExecutorAO()
			throws JargonException {
		if (simpleQueryExcecutor == null) {
			simpleQueryExcecutor = getIRODSAccessObjectFactory()
					.getSimpleQueryExecutorAO(getIRODSAccount());
		}
	 ...
	 * @throws JargonException
	 */
	private Quota buildQuotaFromPerResourceResultRow(
		...
		quotaValue.setQuotaLimit(IRODSDataConversionUtil
				.getLongOrZeroFromIRODSValue(row.getColumn(3)));
		quotaValue.setQuotaOver(IRODSDataConversionUtil
				.getLongOrZeroFromIRODSValue(row.getColumn(4)));
		quotaValue.setUpdatedAt(IRODSDataConversionUtil
				.getDateFromIRODSValue(row.getColumn(5)));
		return quotaValue;
	}

	private Quota buildQuotaFromGlobalResultRow(final IRODSQueryResultRow row)
			throws JargonException {
		Quota quotaValue;
		quotaValue = new Quota();
		quotaValue.setUserName(row.getColumn(0));
		...
		quotaValue.setQuotaLimit(IRODSDataConversionUtil
				.getLongOrZeroFromIRODSValue(row.getColumn(2)));
		quotaValue.setQuotaOver(IRODSDataConversionUtil
				.getLongOrZeroFromIRODSValue(row.getColumn(3)));
		quotaValue.setUpdatedAt(IRODSDataConversionUtil
				.getDateFromIRODSValue(row.getColumn(4)));
		return quotaValue;
	}
