...
public interface DataTransferOperations extends IRODSAccessObject {
	...
	/**
	 * Move a file or collection between two locations in iRODS. This method will
	 * inspect the paths and create the appropriate command to iRODS automatically.
	 * In this method, the target file is expressed as the actual target location.
	 * There are other methods in this class that will take the last part of the
	 * source path, and use that as the collection name in the target.
	 * <p>
	 ...
	 * source file is a data object, and the target file is a collection. In this
	 * case, the file name is propagated as the name of the file under the target
	 * collection.
	 * <p>
	 * If an attempt is made to move a file to itself, the case will be logged and
	 * ignored.
	 *
	 * @param absolutePathToSourceFile
	 *            {@code String} with the absolute path to the source file.
	 * @param absolutePathToTargetFile
	 *            {@code String} with the absolute path to the target of the move.
	 * @throws JargonException
	 *             for iRODS error
	 * @throws JargonFileOrCollAlreadyExistsException
	 *             if a move is made to a file or collection that already exists
	 ...
	 * registration of a {@code TransferStatusCallbackListener} that will provide
	 * call-backs about the status of the transfer suitable for progress monitoring.
	 * <p>
	 * Note that this method will, if the correct jargon properties are set, support
	 * connection re-routing to appropriate resources for the transfer.
	 * <p>
	 * Note: certain extended transfer options for put (such as target data type)
	 * are set in the {@link TransferOptions} object, which is specified by creating
	 * and passing within the {@link TransferControlBlock}. For example, setting a
	 * data object to be an MSSO object is accomplished by setting that type in the
	 * {@link TransferOptions} object.
	 *
	 * @param sourceFile
	 ...
	 *            {@link TransferStatusCallbackListener} an optional listener to
	 ...
	 *            real-time status of the transfer. This may be set to null if not
	 *            required
	 * @param transferControlBlock
	 *            an optional
	 *            {@link org.irods.jargon.core.transfer.TransferControlBlock} that
	 ...
	 * from iRODS.
	 * <p>
	 * Note that the {@code TransferControlBlock} contains a {@code TransferOptions}
	 * object that can control details of transfer behavior for the given transfer.
	 * These options may be set before the transfer begins to control things like
	 * max parallel threads, whether to use connection re-routing, and whether to
	 * use a force option. The force option in this case controls whether the local
	 * file in the get should be over-written if it exists already. The standard
	 ...
	 * get a call to the {@code transferAsksWhetherToForceOperation()}, and the
	 * client may answer with a yes, no, yes to all, and no to all type response. An
	 * 'all' response will alter the {@code TransferOptions} that prevail for the
	 * current transfer for subsequent files in this transfer.
	 *
	 * @param irodsSourceFileAbsolutePath
	 *            {@code String} with the absolute path to the iRODS source file to
	 *            retrieve to the client
	 * @param targetLocalFileAbsolutePath
	 *            {@code String} that is the absolute path to file in the local file
	 *            system to which the iRODS data will be transferred
	 * @param sourceResourceName
	 *            {@code String} with the optional resource from which the file will
	 *            be obtained. This should be left blank if not specified (not null)
	 * @param transferStatusCallbackListener
	 *            {@link org.irods.jargon.core.transfer.TransferStatusCallbackListener}
	...
	/**
	 * Perform a replication operation. This will copy the given file to a target
	 * iRODS resource.
	 *
	 ...
	 *
	 * @param irodsSourceFileAbsolutePath
	 *            {@code String} with the absolute path to the source collection or
	 *            data object. This will be copied up to the target
	 * @param targetResource
	 *            {@code String} with the optional (blank if not specified) resource
	 *            to which the file or collection will be copied
	 * @param irodsTargetFileAbsolutePath
	 *            {@code String} with the absolute path to the target iRODS file or
	 *            collection. A file may be copied to a collection
	 * @param force
	 *            {@code boolean} that indicates that any files that exist in the
	 *            target will be copied over
	 * @param transferStatusCallbackListener
	 *            an optional
	 *            {@link org.irods.jargon.core.transfer.TransferStatusCallbackListener}
	 *            that can receive status callbacks. This may be set to null if this
	...
	/**
	 * Transfer a file from the local file system to iRODS. This will be a recursive
	 * operation if a collection is specified. If a collection is specified, that
	 ...
	 *            {@code String} with the absolute path of the source file on the
	 *            local file system
	 * @param targetIrodsFileAbsolutePath
	 *            {@code String} with the absolute path of the iRODS collection that
	 *            will be the target of the put
	 * @param targetResourceName
	 *            {@code String} with the target resource name. This may be set to
	 *            blank if not used, in which case the iRODS default will be used.
	 ...
	 * {@code TransferStatusCallbackListener} objects are optional and may be set to
	 ...
	 *            collection in iRODS.
	 * @param targetResource
	 *            {@code String} (blank if not defined) with the target resource for
	 *            the copy.
	 * @param irodsTargetFileAbsolutePath
