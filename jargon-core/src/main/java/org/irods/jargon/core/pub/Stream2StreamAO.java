...
/**
 * Interface for an access object that helps with copying streams or byte arrays to and from iRODS files.
 * <p/>
...
public interface Stream2StreamAO {
	 ...
	 * @param bytesToStream <code>byte[]</code> with the bytes to copy
	...
	/**
	 * Stream the file contents to a byte array.  Note that this method is suitable for small data sizes, but
	 * since it uses memory, large files may cause memory problems.  
	 * <p/>
	 * This particular method is used internally for object de-serialization when stored as iRODS files.
	 * 
