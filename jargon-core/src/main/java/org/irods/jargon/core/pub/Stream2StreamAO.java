...
public interface Stream2StreamAO extends IRODSAccessObject {
	...
	/**
	 * Stream the file contents to a byte array. Note that this method is suitable
	 * for small data sizes, but since it uses memory, large files may cause memory
	 * problems.
	 * <p>
	 ...
	 * operations, and will attempt to do connection re-routing for iRODS files if
	 * so specified in the jargon properties.
	 * <p>
	...
	/**
	 * Copy an input stream to an output stream as guided by the jargon.properties
	 * settings.
	 * <p>
	 * Note that this method will buffer the streams provided in the call if they
	 * are not buffered, controlled by the jargon.properties.
	 * <p>
