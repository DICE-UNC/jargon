...
public class IRODSFileImplTest {
	...
	public final void testCanReadViaGroup() throws Exception {
		...
		IRODSFileFactory irodsFileFactory = irodsFileSystem.getIRODSFileFactory(secondaryIrodsAccount);
		IRODSFile actual = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + testFileName);
	...
	public final void testCanReadCollectionViaGroupMembership() throws Exception {
		...
		userGroupAO.addUserToGroup(testColl, secondaryAccount.getUserName(), null);

		collectionAO.removeAccessPermissionForUser(irodsAccount.getZone(), targetIrodsCollection, testColl, false);

		collectionAO.setAccessPermissionRead(irodsAccount.getZone(), targetIrodsCollection, testColl, false);

		IRODSFile actual = accessObjectFactory.getIRODSFileFactory(secondaryAccount)
				.instanceIRODSFile(targetIrodsCollection);
	 ...
	 */
	@Test
	public final void testCanReadCollectionViaGroupMembershipWhenRoot() throws Exception {
		...
		IRODSFile actual = accessObjectFactory.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(targetIrodsCollection);
	...
	/**
	 * Bug https://github.com/DICE-UNC/jargon/issues/10 apparent error getting
	 * isFile for a .gitignore file #10
	 */
	@Test
	public final void testIsFileDotGitignoreBug10() throws Exception {
		String testFileName = ".gitignore";
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH);
		FileGenerator.generateFileOfFixedLengthGivenName(absPath, testFileName, 8);
	...
	public final void testChildrenWhenRootDoesNotContainRoot() throws Exception {
		...
		for (File child : children) {
			Assert.assertFalse("child of root given as root", child.getName().equals("/"));
		}
	...
	public final void testDeleteAFileQuotesInShorterFileName() throws Exception {
		...
		DataTransferOperations dataTransferOperations = irodsFileSystem.getIRODSAccessObjectFactory()
				.getDataTransferOperations(irodsAccount);
		dataTransferOperations.putOperation(sourceFile, targetIRODSFile, null, null);
	...
	public final void testDeleteACollectionWithAFileWithQuotesInLongerFileName() throws Exception {
		...
		IRODSFile targetIRODSCollection = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(targetIrodsCollection);
	 ...
	 */
	@Test
	public final void testDeleteACollectionWithSpacesInTheNameName() throws Exception {
	...
	public final void testIsFileWhenNotExists() throws Exception {
		...
		IRODSFileFactory irodsFileFactory = accessObjectFactory.getIRODSFileFactory(irodsAccount);
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + "/blah");
		boolean isFile = irodsFile.isFile();
	...
	public final void testLength() throws Exception {
		...
		int expectedSize = 8;
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH);
		FileGenerator.generateFileOfFixedLengthGivenName(absPath, testFileName, expectedSize);
	...
	public final void testCreateNewCollectionGivingParentAndChild() throws Exception {
		...
		IRODSAccessObjectFactory accessObjectFactory = irodsFileSystem.getIRODSAccessObjectFactory();
		IRODSFileFactory irodsFileFactory = accessObjectFactory.getIRODSFileFactory(irodsAccount);
		IRODSFile delFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + "/" + testNewChild);
		delFile.deleteWithForceOption();
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection, testNewChild);
		boolean success = irodsFile.mkdirs();
		Assert.assertEquals("path set correctly", targetIrodsCollection + "/" + testNewChild,
	...
	public final void testDeleteFile() throws Exception {
		...
		assertionHelper.assertIrodsFileOrCollectionDoesNotExist(irodsFile.getAbsolutePath(), accessObjectFactory,
				irodsAccount);
	}
	 ...
	 */
	@Test
	public final void testDeleteFileWithForceBug216() throws Exception {
		String testCollectionName = "testDeleteFileWithForceBug216coll";
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH);
	...
	public final void testDeleteFileWhenCollection() throws Exception {
		...
		Assert.assertTrue("did not get a true result from the collection delete", result);
		assertionHelper.assertIrodsFileOrCollectionDoesNotExist(irodsFile.getAbsolutePath(),
				irodsFileSystem.getIRODSAccessObjectFactory(), irodsAccount);
	}
	...
	public final void testCompareToFile() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + testFileName);
		IRODSFile irodsFile2 = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + testFileName2);
	...
	public final void testEqualsObject() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection);
		IRODSFile irodsFile2 = irodsFileFactory.instanceIRODSFile(targetIrodsCollection);
	...
	public final void testNotEqualsObject() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection);
		IRODSFile irodsFile2 = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + "xxx");

		irodsSession.closeSession();

		Assert.assertFalse("files should not be equal", irodsFile.equals(irodsFile2));
	}
	...
	public final void testGetAbsoluteFile() throws Exception {
		...
		Assert.assertEquals("absolute paths must match", irodsFile.getAbsolutePath(), newIRODSFile.getAbsolutePath());
	...
	public final void testGetParent() throws Exception {
		...
		Assert.assertEquals("parent names do not match", targetIrodsCollection, actualParent);
	}
	...
	public final void testGetParentWhenSubdirBelowRoot() throws Exception {
		...
		Assert.assertEquals("should have gotten root as parent", "/", actualParent);
	}
	...
	public final void testGetParentFile() throws Exception {
		...
		Assert.assertEquals("files", irodsFile.getParent(), irodsParentFile.getAbsolutePath());
	}
	...
	public final void testListFiles() throws Exception {
		...
		for (File irodsFile2 : irodsFiles) {
			Assert.assertTrue("this is not an instance of IRODSFileImpl", irodsFile2 instanceof IRODSFile);
		}
	}
	...
	public final void testListFilesFilenameFilter() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection);
		File[] irodsFiles = irodsFile.listFiles(new IRODSAcceptAllFileNameFilter());
		Assert.assertNotNull(irodsFiles);
		Assert.assertTrue("no results", irodsFiles.length > 0);
		for (File irodsFile2 : irodsFiles) {
			Assert.assertTrue("this is not an instance of IRODSFileImpl", irodsFile2 instanceof IRODSFile);
	...
	/**
	 * Test for bug move versus rename semantics, collection already exists on
	 * rename collection #161 https://github.com/DICE-UNC/jargon/issues/161
	 *
	 * @throws Exception
	 */
	@Test
	public void testRenameCollectionBug161() throws Exception {
	...
	public final void testRenameToFileFilePhyMove() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + testFileName);
		IRODSFile irodsRenameFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + testFileName);
		irodsRenameFile
	...
	public final void testToURI() throws Exception {
		...
		String expectedString = "irods://" + irodsAccount.getUserName() + "@" + irodsAccount.getHost() + ":"
				+ irodsAccount.getPort() + irodsFile.getAbsolutePath();
		Assert.assertEquals("to string does not match expected", expectedString, actualString);
	}
	...
	public final void testToString() throws Exception {
		...
		Assert.assertEquals("host not equal", irodsAccount.getHost(), actualHost);
		Assert.assertEquals("port not equal", irodsAccount.getPort(), actualPort);
		Assert.assertEquals("path not equal", irodsFile.getAbsolutePath(), actualPath);
	}
	...
	public final void testGetResourceDirectory() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + testDirName);

		String resource = irodsFile.getResource();

		Assert.assertEquals("did not get expected resource for directory", "", resource);
	}
