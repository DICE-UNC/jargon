...
public class DataTransferOperationsImplTest {
	...
	public void testPhysicalMoveDelegatedThroughMove() throws Exception {
		...
		String testResource = testingProperties.getProperty(TestingPropertiesHelper.IRODS_SECONDARY_RESOURCE_KEY);
	...
	public void testMoveFileToSelf() throws Exception {
		...
		dataTransferOperations.move(irodsFile.getAbsolutePath(), irodsFile.getAbsolutePath());

		IRODSFile actualFile = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(irodsFile.getAbsolutePath());

		Assert.assertTrue("source file should still be in place", actualFile.exists());
	...
	public void testMoveCollection() throws Exception {
		...
		IRODSFile irodsOrigFile = irodsFileSystem.getIRODSFileFactory(irodsAccount)
		...
		dataTransferOperations.putOperation(localCollectionAbsolutePath, irodsFile.getAbsolutePath(), "", null, null);

		dataTransferOperations.move(irodsOrigFile.getAbsolutePath(), irodsDestFile.getAbsolutePath());
	...
	public void testPutOneFileIntraFileCallbacksSpecifiedJargonPropsAndVerified() throws Exception {
		...
		dataTransferOperationsAO.putOperation(localFile, destFile, testingCallbackListener, null);
		assertionHelper.assertIrodsFileMatchesLocalFileChecksum(destFile.getAbsolutePath(), localFile.getAbsolutePath(),
				irodsFileSystem.getIRODSAccessObjectFactory(), irodsAccount);
		// last intra callback should have total length (100%)
		Assert.assertEquals("intra file last call not 100%", localFile.length(),
				testingCallbackListener.getBytesReportedIntraFileCallbacks());

		Assert.assertFalse("no bytes reported in callbacks",
	...
	public void testPutOneFileIntraFileCallbacksSpecifiedTransferOptsAndVerified() throws Exception {
		...
		dataTransferOperationsAO.putOperation(localFile, destFile, testingCallbackListener, tcb);
		assertionHelper.assertIrodsFileMatchesLocalFileChecksum(destFile.getAbsolutePath(), localFile.getAbsolutePath(),
				irodsFileSystem.getIRODSAccessObjectFactory(), irodsAccount);
	...
	public void testPutOneFileNoParallelBug1837() throws Exception {
		...
		SettableJargonProperties props = new SettableJargonProperties(jargonOriginalProperties);
	...
	public void testPutOneFileWithResourceRerouting() throws Exception {
		...
		Assert.assertEquals("did not get expected resource", 1, resources.size());
		Resource firstResource = resources.get(0);
		Assert.assertEquals("resource for file not correct", destFile.getResource(), firstResource.getName());
	...
	@Test
	public void testGetOneFileWithIntraFileInPropsVerifyCallbacks() throws Exception {
		// generate a local scratch file
		...
		Assert.assertEquals("did not get the full local file name in callback", getLocalFile.getAbsolutePath(),
				testCallbackListener.getLastTargetPath());
		Assert.assertFalse("no intra file callbacks", testCallbackListener.getNumberIntraFileCallbacks() == 0);
	...
	@Test
	public void testGetOneFileWithCallbackAndControlBlockCheckOneFileToBeTransferred() throws Exception {
		// generate a local scratch file
		...
		// now get the file
		dataTransferOperationsAO.getOperation(getIRODSFile, getLocalFile, testCallbackListener, transferControlBlock);

		Assert.assertEquals("Should have counted 1 file to transfer", 1,
				transferControlBlock.getTotalFilesToTransfer());
		Assert.assertEquals("should have 1 file transferred as reflected in transferControlBlock", 1,
				transferControlBlock.getTotalFilesTransferredSoFar());
		Assert.assertTrue("No errors should have been accumulated in the transferControlBlock",
	...
	public void testGetWithCancelThenRestart() throws Exception {
		...
		FileGenerator.generateManyFilesInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testPutWithCancel", ".txt", 10, 1, 2);
		...
		dataTransferOperationsAO.getOperation(destFile.getAbsolutePath() + "/" + rootCollection,
				scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH + "/" + getToCollection), "",
				listener, transferControlBlock);

		TransferControlBlock restartControlBlock = irodsFileSystem.getIrodsSession()
				.buildDefaultTransferControlBlockBasedOnJargonProperties();
		restartControlBlock.getTransferOptions().setForceOption(ForceOption.USE_FORCE);
		dataTransferOperationsAO.getOperation(destFile.getAbsolutePath() + "/" + rootCollection,
				scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH + "/" + getToCollection), "",
				null, restartControlBlock);

		File getToTargetFile = new File(scratchFileUtils.createAndReturnAbsoluteScratchPath(
				IRODS_TEST_SUBDIR_PATH + "/" + getToCollection + "/" + rootCollection));

		Assert.assertEquals("should have 10 files in coll after restart", 10, getToTargetFile.list().length);
		Assert.assertEquals("should have counted 10 files", 10, restartControlBlock.getTotalFilesTransferredSoFar());
	}
	...
	public void testGetOneFileWithNoCallbackAndControlBlock() throws Exception {
		...
		Assert.assertEquals("Should not have counted transfers even if no status callback", 1,
				transferControlBlock.getTotalFilesToTransfer());
	}
	...
	public void testPutCollectionWithZeroLengthFilesInIt() throws Exception {
		...
		int nbrFiles = 10;
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH + "/" + testColl);
		...
		destFile = irodsFileFactory.instanceIRODSFile(targetIrodsFile, testColl);
	...
	public void testGetCollectionWithTwoFilesWithNoCallbacksNoPermission() throws Exception {
		...
		dataTransferOperationsAO = irodsFileSystem.getIRODSAccessObjectFactory()
				.getDataTransferOperations(secondaryAccount);
	 ...
	 */
	@Test
	public void testGetCollectionWithTwoFilesWithCallbacksNoPermission() throws Exception {
		...
		dataTransferOperationsAO.getOperation(getIrodsFile, returnLocalFile, testCallbackListener, null);

		Assert.assertTrue("did not get errors from callbacks", testCallbackListener.getErrorCallbacks().size() > 0);
	...
	public void testGetCollectionWithOverwriteNoForce() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testGetCollectionWithOverwriteNoForce", 1, 1, 1, "testFile", ".txt", 5, 5, 1, 2);
		FileGenerator.generateFileOfFixedLengthGivenName(localCollectionAbsolutePath, overwriteFileName, 1);
		...
		dataTransferOperationsAO.putOperation(localFile, destFile, null, null);
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(
				IRODS_TEST_SUBDIR_PATH + "/" + returnedLocalCollection + "/" + rootCollection);
		...
		FileGenerator.generateFileOfFixedLengthGivenName(absPath, overwriteFileName, 1);
		...
		dataTransferOperationsAO.getOperation(getIrodsFile, returnLocalFile, null, tcb);
	...
	@Test
	public void testPutMultipleCollectionsMultipleFilesWithRenewalTurnedOn() throws Exception {
	 ...
	 */
	@Test(expected = OverwriteException.class)
	public void testPutCollectionWhereOneFileIsOverwriteNoForceSpecified() throws Exception {
		...
		String absPath = scratchFileUtils
				.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH + "/" + testCollectionSubdir + "staging");
		String computedFileName = filePrefix + "2" + fileSuffix;
		FileGenerator.generateFileOfFixedLengthGivenName(absPath, computedFileName, 3);
		...
		IRODSFile destFile = irodsFileSystem.getIRODSFileFactory(irodsAccount).instanceIRODSFile(targetIrodsCollection);
		...
		for (int i = 0; i < nbrFilesInDir; i++) {
			...
			FileGenerator.generateFileOfFixedLengthGivenName(absPath, genFileName, i + 10);
		}
		...
		destFile = irodsFileSystem.getIRODSFileFactory(irodsAccount).instanceIRODSFile(targetIrodsCollection);
		dataTransferOperationsAO.putOperation(localFile, destFile, null, tcb);
	 ...
	 */
	@Test
	public void testPutCollectionWhereOneFileIsOverwriteAskCallbackListenerRespondsNo() throws Exception {
		...
		String absPath = scratchFileUtils
				.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH + "/" + testCollectionSubdir + "staging");
		String computedFileName = filePrefix + "2" + fileSuffix;
		FileGenerator.generateFileOfFixedLengthGivenName(absPath, computedFileName, firstLength);
		...
		for (int i = 0; i < nbrFilesInDir; i++) {
			genFileName = filePrefix + i + fileSuffix;
			FileGenerator.generateFileOfFixedLengthGivenName(absPath, genFileName, secondLength);
		}
		...
		targetIrodsCollection = testingPropertiesHelper
				.buildIRODSCollectionAbsolutePathFromTestProperties(testingProperties, IRODS_TEST_SUBDIR_PATH);
		destFile = irodsFileSystem.getIRODSFileFactory(irodsAccount).instanceIRODSFile(targetIrodsCollection);
		dataTransferOperationsAO.putOperation(localFile, destFile, transferStatusCallbackListener, tcb);

		// make sure chosen file was not overwritten
		IRODSFile compareIrods = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(targetIrodsCollection + "/" + testCollectionSubdir, computedFileName);
	 ...
	 */
	@Test
	public void testPutCollectionWhereOneFileIsOverwriteAskCallbackListenerRespondsYes() throws Exception {
	 ...
	 */
	@Test
	public void testPutCollectionWhereOneFileIsOverwriteForceSpecified() throws Exception {
	...
	@Test
	public void testPutThenGetMultipleCollectionsMultipleFilesWithCallbacks() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testPutThenGetMultipleCollectionsMultipleFiles", 1, 2, 2, "testFile", ".txt", 4, 2, 20, 200);
		...
		File returnLocalFile = new File(returnCollectionAbsolutePath);
		destFile = irodsFileFactory.instanceIRODSFile(irodsCollectionRootAbsolutePath + '/' + rootCollection);
		dataTransferOperationsAO.getOperation(destFile, returnLocalFile, listener, null);
		...
		Assert.assertEquals(listener.getGetCallbackCtr(), listener.getPutCallbackCtr());
	}

	@Test
	public void testPutMultipleCollectionsMultipleFilesWithCallbacksAndControlBlock() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testPutThenGetMultipleCollectionsMultipleFiles", 2, 3, 2, "testFile", ".txt", 4, 2, 20, 200);
		...
		dataTransferOperationsAO.putOperation(localFile, destFile, listener, transferControlBlock);

		Assert.assertTrue("did not do a pre-count of files", transferControlBlock.getTotalFilesToTransfer() > 0);
		Assert.assertEquals("did not transfer the amount of files I pre-counted",
				transferControlBlock.getTotalFilesToTransfer(), transferControlBlock.getTotalFilesTransferredSoFar());
	}
	...
	public void testPutWithCancel() throws Exception {
		...
		Assert.assertEquals("should have  3 files in coll before cancelled", 3, destFile.list().length);
		Assert.assertTrue("should have a status callback of cancelled", listener.isCancelEncountered());
	}
	...
	public void testPutWithSkip() throws Exception {
		...
		FileGenerator.generateManyFilesInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testPutWithCancel", ".txt", fileCtr, 1, 2);
		...
		Assert.assertEquals("didnt get an init callback for each file", fileCtr, listener.getInitCallbackCtr());
		Assert.assertEquals("didnt get skips for half", expectedSkipped, listener.getSkipCtr());
		Assert.assertEquals("didnt get transfers for half", expectedSkipped, listener.getPutCallbackCtr());
	...
	public void testPutWithCancelThenRestart() throws Exception {
		...
		dataTransferOperationsAO.putOperation(localFile, destFile, listener, restartControlBlock);

		IRODSFile destTarget = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(destFile.getAbsolutePath() + "/" + rootCollection);

		Assert.assertEquals("should have 9 files in coll after restart", 9, destTarget.list().length);
		Assert.assertEquals("should have counted 10 files", 10, restartControlBlock.getTotalFilesTransferredSoFar());
	}
	...
	public void testPutWithPause() throws Exception {
		...
		FileGenerator.generateManyFilesInParentCollectionByAbsolutePath(localCollectionAbsolutePath, "testPutWithPause",
				".txt", 10, 1, 2);
		...
		Assert.assertEquals("wrong number of files in coll before pause", 2, destFile.list().length);
		Assert.assertTrue("should have a status callback of pause", listener.isPauseEncountered());
	}
	...
	@Test
	public void testReplicateMultipleCollectionsMultipleFilesWithControlBlock() throws Exception {
		...
		// now replicate
		dataTransferOperationsAO.replicate(irodsCollectionRootAbsolutePath + "/" + rootCollection, targetResource, null,
				transferControlBlock);

		Assert.assertTrue("did not pre-count files to replicate", transferControlBlock.getTotalFilesToTransfer() > 0);
		Assert.assertEquals("did not count files as replicated to match pre-count total",
				transferControlBlock.getTotalFilesToTransfer(), transferControlBlock.getTotalFilesTransferredSoFar());
	...
	public void testReplicateWithCancel() throws Exception {
		...
		FileGenerator.generateManyFilesInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testReplicateWithCancel", ".txt", 10, 1, 2);
		...
		Assert.assertTrue("did not get expected success callback", listener.getReplicateCallbackCtr() == 4);
		Assert.assertTrue("should have a status callback of cancelled", listener.isCancelEncountered());
	...
	public void testReplicateWithCancelThenRestart() throws Exception {
		...
		listener = new TransferStatusCallbackListenerTestingImplementation(transferControlBlock, 0, 0);

		dataTransferOperationsAO.replicate(irodsCollectionRootAbsolutePath + "/" + rootCollection, targetResource,
				listener, transferControlBlock);

		// assess the callbacks
		Assert.assertEquals("should have counted 10 files", 10, transferControlBlock.getTotalFilesTransferredSoFar());
	...
	public void testReplicateWithPause() throws Exception {
		...
		FileGenerator.generateManyFilesInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testReplicateWithPause", ".txt", 10, 1, 2);
		...
		Assert.assertTrue("did not get expected success callback", listener.getReplicateCallbackCtr() == 4);
		Assert.assertTrue("should have a status callback of paused", listener.isPauseEncountered());
	...
	@Test
	public void testPutCollectionWithTwoFilesControlBlockNoCallback() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testPutCollectionWithTwoFilesControlBlockNoCallback", 1, 1, 1, "testFile", ".txt", 2, 2, 1, 2);
	...
	@Test
	public void testPutCollectionWithTwoFilesNoControlBlockCallback() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testPutCollectionWithTwoFilesNoControlBlockCallback", 1, 1, 1, "testFile", ".txt", 2, 2, 1, 2);
	...
	public void testGetWithCancel() throws Exception {
		...
		dataTransferOperationsAO.getOperation(getIrodsFile, returnLocalFile, listener, transferControlBlock);
		...
		// I've put a mess of stuff, now get it
		Assert.assertEquals("wrong number of files in coll before cancelled", 3, returnCompareLocalFile.list().length);
		Assert.assertTrue("should have a status callback of cancelled", listener.isCancelEncountered());
	}

	@Test
	public void testGetWithCallbacksMultipleLevelsOfCollections() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				rootCollection, 3, 4, 2, "prefix", ".suffix", 7, 3, 10, 300);
	...
	@Test
	public void testGetWithCallbacksMultipleLevelsOfCollectionsVerifyCountsInControlBlock() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				rootCollection, 2, 3, 2, "prefix", ".suffix", 7, 2, 1000, 30000);
		...
		Assert.assertEquals("did not get count matching expected files in the transferControlBlock",
				transferControlBlock.getTotalFilesToTransfer(), transferControlBlock.getTotalFilesTransferredSoFar());
	...
	public void testGetWithSkipOfEvenFiles() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				rootCollection, 3, 4, 2, "prefix", ".suffix", 7, 2, 1, 3);
		...
		Assert.assertEquals("transfer controls out of balance", listener.getInitCallbackCtr(),
				transferControlBlock.getTotalFilesTransferredSoFar());

		Assert.assertEquals("skip counts disagree between listener and transferControlBlock", listener.getSkipCtr(),
	 ...
	 */
	@Test
	public void testMoveSourceCollectionTargetCollectionBug63() throws Exception {
		...
		String irodsCollectionRoot2AbsolutePath = testingPropertiesHelper
				.buildIRODSCollectionAbsolutePathFromTestProperties(testingProperties,
						IRODS_TEST_SUBDIR_PATH + "/" + rootCollection2);

		IRODSAccount irodsAccount = testingPropertiesHelper.buildIRODSAccountFromTestProperties(testingProperties);

		IRODSFileFactory irodsFileFactory = irodsFileSystem.getIRODSFileFactory(irodsAccount);

		IRODSFile rootFile = irodsFileFactory.instanceIRODSFile(irodsCollectionRootAbsolutePath);
		IRODSFile root2File = irodsFileFactory.instanceIRODSFile(irodsCollectionRoot2AbsolutePath);

		rootFile.mkdirs();

		FileGenerator.generateFileOfFixedLengthGivenName(localCollectionAbsolutePath, testFileName, 10);

		DataTransferOperations dataTransferOperationsAO = irodsFileSystem.getIRODSAccessObjectFactory()
				.getDataTransferOperations(irodsAccount);
		File localFile = new File(localCollectionAbsolutePath + "/" + testFileName);
	...
	public void testMoveSourceCollectionTargetCollectionBug277() throws Exception {
		...
		String irodsCollectionTargetAbsolutePath = testingPropertiesHelper
				.buildIRODSCollectionAbsolutePathFromTestProperties(testingProperties,
						IRODS_TEST_SUBDIR_PATH + "/" + targetCollection);

		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"testMoveCollectionWithTwoFilesUnderneathAParent", 1, 1, 1, "testFile", ".txt", 2, 2, 1, 2);
		...
		irodsFileSystem = IRODSFileSystem.instance();
		irodsFileFactory = irodsFileSystem.getIRODSFileFactory(irodsAccount);
		destFile = irodsFileFactory.instanceIRODSFile(irodsCollectionTargetAbsolutePath);

		Assert.assertTrue("did not find expected targetcollection", destFile.isDirectory());
	}

	@Test
	public void testMoveFileSpecifyCollectionAsTargetSourceFileHasNoExtension() throws Exception {
		...
		dto.move(irodsFile.getAbsolutePath(), targetCollectionFile.getAbsolutePath());
	...
	public void testGetJargonProperties() throws Exception {
		...
		DataTransferOperations dataTransferOperationsAO = irodsFileSystem.getIRODSAccessObjectFactory()
				.getDataTransferOperations(irodsAccount);
		JargonProperties jargonProperties = dataTransferOperationsAO.getJargonProperties();
	...
	public void testCopyFileToCollectionTargetBug142() throws Exception {
		...
		dataTransferOperationsAO.putOperation(localFile, destFile, null, null);
		String irodsTargetAbsolutePath = testingPropertiesHelper.buildIRODSCollectionAbsolutePathFromTestProperties(
				testingProperties, IRODS_TEST_SUBDIR_PATH + "/" + testTargetDirectory);

		dataTransferOperationsAO.copy(targetIrodsFile, "", irodsTargetAbsolutePath + "/" + testFileName, null, null);

		IRODSFile targetFile = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(irodsTargetAbsolutePath + "/" + testFileName);
	...
	public void testCopyCollectionToTargetCollection() throws Exception {
		...
		String irodsOriginalAbsolutePath = testingPropertiesHelper.buildIRODSCollectionAbsolutePathFromTestProperties(
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				"prefixForColl", 2, 3, 2, "testFile", ".txt", 2, 2, 1, 2);
		...
		dataTransferOperations.putOperation(localCollectionAbsolutePath, irodsCollectionRootAbsolutePath, "", null,
				null);

		dataTransferOperations.copy(irodsOriginalAbsolutePath, "", irodsTargetAbsolutePath, null, null);
	...
	public void testCopyCollectionToTargetCollectionSelfBug151() throws Exception {
	...
	@Test
	public void testCopyCollectionToTargetCollectionWithResource() throws Exception {
		...
		dataTransferOperations.copy(irodsOriginalAbsolutePath, targetResource, irodsTargetAbsolutePath, null, null);
		...
		Assert.assertEquals("did not set correct resource", targetResource, foundResc);

	}

	@Test
	public void testCopyCollectionToTargetCollectionWithResourceInIrodsAccount() throws Exception {
	...
	private String getResourceFromFiles(final IRODSFile parent, final DataObjectAO dataObjectAO) throws Exception {
		String resource = "";
		for (File child : parent.listFiles()) {
			if (child.isFile()) {
				List<Resource> resources = dataObjectAO.listFileResources(child.getAbsolutePath());
	 ...
	 */
	@Test
	public void testCopyCollectionToTargetCollectionOverwriteForce() throws Exception {
		...
		dataTransferOperations.copy(irodsOriginalAbsolutePath, "", irodsTargetAbsolutePath, null, null);
		dataTransferOperations.copy(irodsOriginalAbsolutePath, "", irodsTargetAbsolutePath, null, tcb);
	...
	public final void testCopyIRODSDataObjectToDataObjectNoForceWhenOverwrite() throws Exception {
		...
		DataTransferOperations dataTransferOperations = irodsFileSystem.getIRODSAccessObjectFactory()
				.getDataTransferOperations(irodsAccount);
		dataTransferOperations.copy(irodsSourceFile, irodsTargetFile, null, tcb);
		dataTransferOperations.copy(irodsSourceFile, irodsTargetFile, null, tcb);
	 ...
	 */
	@Test
	public final void testCopyIRODSDataObjectToDataObjectNoForceNoOverwriteStringSignatures() throws Exception {
		...
		dataObjectAO.putLocalDataObjectToIRODS(new File(fileNameOrig), irodsFile, null, null, true);
		IRODSFile irodsSourceFile = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(targetIrodsCollection + "/" + testFileName);
		IRODSFile irodsTargetFile = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(targetIrodsCollection + "/" + testCopyToFileName);
		...
		DataTransferOperations dataTransferOperations = irodsFileSystem.getIRODSAccessObjectFactory()
				.getDataTransferOperations(irodsAccount);
		dataTransferOperations.copy(irodsSourceFile.getAbsolutePath(), "", irodsTargetFile.getAbsolutePath(), null,
				tcb);
	...
	@Test
	public void testCopyCollectionWithAnEmptyChildCollectionToTarget() throws Exception {
		...
		testDir = irodsFileSystem.getIRODSFileFactory(irodsAccount).instanceIRODSFile(irodsOriginalAbsolutePath,
				testOrigSubdir);
		testDir.mkdirs();
		...
		IRODSFile targetDir = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(irodsTargetAbsolutePath, testOrigDirectory);
		Assert.assertTrue("did not find copied directory under target", targetDir.exists() && targetDir.isDirectory());
		targetDir = irodsFileSystem.getIRODSFileFactory(irodsAccount).instanceIRODSFile(targetDir.getAbsolutePath(),
				testOrigSubdir);
		Assert.assertTrue("did not find child of copied directory under target",
	...
	public void testCopyCollectionToSelfParentWhenFile() throws Exception {
		...
		dataTransferOperations.putOperation(fileNameOrig, irodsCollectionRootAbsolutePath, "", null, null);

		dataTransferOperations.copy(irodsCollectionRootAbsolutePath + "/" + testFileName, "",
				irodsCollectionRootAbsolutePath, null, false, null);
	}
	...
	public void testCopyCollectionWithCancelThenRestart() throws Exception {
		...
		dataTransferOperationsAO.copy(destFile.getAbsolutePath() + "/" + rootCollection, "",
				irodsCollectionTargetAbsolutePath, listener, false, transferControlBlock);

		Assert.assertEquals("did not hit cancel as anticipated in copy", 2,
				transferControlBlock.getTotalFilesTransferredSoFar());

		// now do a restart and complete the copy
		IRODSFile copySourceFiles = irodsFileSystem.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(destFile.getAbsolutePath() + "/" + rootCollection);
		...
		Assert.assertEquals("did not get expected number of restarting callbacks", 3, countRestarted);
		Assert.assertEquals("did not get expected number of success callbacks", 7, countSuccess);
	...
	public void testLongFileNameAddAndDeleteBugDataObjectNameIsLong1044() throws Exception {
		...
		String localFileName = FileGenerator.generateFileOfFixedLengthGivenName(absPath, localTestFileName, 2);
		...
		IRODSFile filePutToIrods = irodsFileSystem.getIRODSFileFactory(irodsAccount)
