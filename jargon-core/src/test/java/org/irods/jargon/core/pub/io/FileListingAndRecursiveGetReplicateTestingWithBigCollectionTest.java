...
public class FileListingAndRecursiveGetReplicateTestingWithBigCollectionTest {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		FileGenerator.generateManyFilesInGivenDirectory(IRODS_TEST_SUBDIR_PATH + '/' + COLL_DIR, testFilePrefix,
				testFileSuffix, NUMBER_OF_TEST_FILES, 1, 2);
		...
		dto.putOperation(absPath + COLL_DIR, targetIrodsCollection,
				testingProperties.getProperty(TestingPropertiesHelper.IRODS_RESOURCE_KEY), null, null);

		IRODSFile collectionRoot = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + "/" + COLL_DIR);

		// make a large number of collections too
		for (int i = 0; i < NUMBER_OF_TEST_FILES; i++) {
			IRODSFile newDir = irodsFileFactory
					.instanceIRODSFile(collectionRoot.getAbsolutePath() + "/" + SUBDIR_NAME + i);
			newDir.mkdir();
		}
	}
	...
	public void testFileList() throws Exception {
		...
		String targetIrodsCollection = testingPropertiesHelper.buildIRODSCollectionAbsolutePathFromTestProperties(
				testingProperties, IRODS_TEST_SUBDIR_PATH + "/" + COLL_DIR);
		IRODSAccount irodsAccount = testingPropertiesHelper.buildIRODSAccountFromTestProperties(testingProperties);
		IRODSFileFactory irodsFileFactory = irodsFileSystem.getIRODSFileFactory(irodsAccount);
		IRODSFile collectionRoot = irodsFileFactory.instanceIRODSFile(targetIrodsCollection);
		String[] names = collectionRoot.list();
		Assert.assertNotNull("no names returned", names);
		Assert.assertEquals("did not get all files", NUMBER_OF_TEST_FILES * 2, names.length);
	...
	public void testFileListFiles() throws Exception {
		...
		Assert.assertEquals("did not get all files", NUMBER_OF_TEST_FILES * 2, files.length);
