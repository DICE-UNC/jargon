...
public class IRODSFileSystemAOImplTest {
	...
	public final void testIsFileNotReadable() throws Exception {
		...
		irodsAccount = testingPropertiesHelper.buildIRODSAccountFromSecondaryTestProperties(testingProperties);
	...
	public final void testGetListInDirReplicatedFiles() throws Exception {
		...
		dataObjectAO.replicateIrodsDataObject(targetIrodsFile,
		...
		Assert.assertEquals("should not display two files, one is a replica", 1, children.length);
	...
	public final void testGetListInDirWithFileFilter() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection);
		IRODSFileSystemAO fileSystemAO = accessObjectFactory.getIRODSFileSystemAO(irodsAccount);
		List<File> subdirs = fileSystemAO.getListInDirWithFileFilter(irodsFile, new IRODSAcceptAllFileFilter());
		Assert.assertNotNull(subdirs);
		Assert.assertTrue("no results", subdirs.size() > 0);
	...
	public final void testGetListFromPreparedSubdir() throws Exception {
		...
		IRODSFile topDir = irodsFileFactory.instanceIRODSFile(targetIrodsCollection, topLevelTestDir);
		topDir.mkdirs();

		// + 2 subdirs

		IRODSFile dir = irodsFileFactory.instanceIRODSFile(targetIrodsCollection, topLevelTestDir + '/' + subdir1);
		dir.mkdirs();

		dir = irodsFileFactory.instanceIRODSFile(targetIrodsCollection, topLevelTestDir + '/' + subdir2);
		dir.mkdirs();
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + topLevelTestDir);
		IRODSFileSystemAO fileSystemAO = accessObjectFactory.getIRODSFileSystemAO(irodsAccount);
		List<String> subdirs = fileSystemAO.getListInDir(irodsFile);
		Assert.assertNotNull(subdirs);
	...
	public final void testGetListFilteredFromPreparedSubdir() throws Exception {
		...
		dirFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + topLevelTestDir + '/' + subdir1);
		dirFile.mkdirs();

		dirFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + topLevelTestDir + '/' + subdir2);
		dirFile.mkdirs();
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection);
		IRODSFileSystemAO fileSystemAO = accessObjectFactory.getIRODSFileSystemAO(irodsAccount);
		List<String> subdirs = fileSystemAO.getListInDirWithFilter(irodsFile, new IRODSAcceptAllFileNameFilter());
		Assert.assertNotNull(subdirs);
		Assert.assertTrue("no results", subdirs.size() == 3);
	}
	...
	public void testFileGetDataType() throws Exception {
		...
		Assert.assertEquals("did not get expected data type for a file", ObjectType.DATA_OBJECT, dataType);
	}
	...
	public void testFileGetDataTypeDirectory() throws Exception {
		...
		Assert.assertEquals("did not get expected data type for a directory", ObjectType.COLLECTION, dataType);
	}
	...
	public final void testFileCreate() throws Exception {
		...
		IRODSFileSystemAO fileSystemAO = accessObjectFactory.getIRODSFileSystemAO(irodsAccount);
		int fileNbr = fileSystemAO.createFile(targetIrodsCollection + '/' + testFileName,
				DataObjInp.OpenFlags.READ_WRITE, DataObjInp.DEFAULT_CREATE_MODE);
		Assert.assertTrue("should have received a file number for the create", fileNbr > 0);
	...
	public void testCreateFileThatAlreadyExists() throws Exception {
		...
		IRODSFileSystemAO fileSystemAO = accessObjectFactory.getIRODSFileSystemAO(irodsAccount);
		fileSystemAO.createFile(targetIrodsCollection + '/' + testFileName, DataObjInp.OpenFlags.READ_WRITE,
	...
	public void testFileDeleteForce() throws Exception {
		...
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH);
		FileGenerator.generateFileOfFixedLengthGivenName(absPath, testFileName, 2);
		String targetIrodsCollection = testingPropertiesHelper
				.buildIRODSCollectionAbsolutePathFromTestProperties(testingProperties, IRODS_TEST_SUBDIR_PATH);
	...
	public void testDirectoryDeleteForce() throws Exception {
		...
		String targetIrodsCollection = testingPropertiesHelper.buildIRODSCollectionAbsolutePathFromTestProperties(
				testingProperties, IRODS_TEST_SUBDIR_PATH + '/' + testDirectoryName);
	...
	public final void testRenameDirectory() throws Exception {
		...
		IRODSFile irodsFile = irodsFileFactory.instanceIRODSFile(targetIrodsCollection + '/' + testDirectory);
		irodsFile.mkdirs();
		IRODSFile irodsRenameFile = irodsFileFactory
				.instanceIRODSFile(targetIrodsCollection + '/' + testRenamedDirectory);
		IRODSFileSystemAO fileSystemAO = accessObjectFactory.getIRODSFileSystemAO(irodsAccount);
	...
	public final void testPhysicalMove() throws Exception {
		...
		fileSystemAO.physicalMove(irodsFile,
				testingProperties.getProperty(TestingPropertiesHelper.IRODS_SECONDARY_RESOURCE_KEY));

		DataObjectAO dataObjectAO = accessObjectFactory.getDataObjectAO(irodsAccount);
		DataObject dataObject = dataObjectAO.findByAbsolutePath(irodsFile.getAbsolutePath());
		Assert.assertEquals("not moved to new resource",
				testingProperties.getProperty(TestingPropertiesHelper.IRODS_SECONDARY_RESOURCE_KEY),
