...
public class IRODSGenQueryExecutorImplBuilderQueriesTest {
	...
	public final void testExecuteQueryWithCount() throws Exception {
		...
		builder.addSelectAsAgregateGenQueryValue(RodsGenQueryEnum.COL_COLL_ID, SelectFieldTypes.COUNT);
	...
	public final void testExecuteMetadataQueryWithIn() throws Exception {
		...
		AvuData avuData = AvuData.instance(expectedAttribName1, expectedAttribValue1, expectedAttribUnits1);
		...
		queryBuilder.addConditionAsGenQueryField(RodsGenQueryEnum.COL_META_DATA_ATTR_NAME,
		...
		queryBuilder.addConditionAsMultiValueCondition(RodsGenQueryEnum.COL_META_DATA_ATTR_VALUE,
				QueryConditionOperators.IN, values);

		queryBuilder.addConditionAsGenQueryField(RodsGenQueryEnum.COL_META_DATA_ATTR_NAME,
				QueryConditionOperators.EQUAL, expectedAttribName2);
		queryBuilder.addConditionAsGenQueryField(RodsGenQueryEnum.COL_META_DATA_ATTR_VALUE,
	...
	public final void testExecuteMetadataQueryWithBetween() throws Exception {
		...
		queryBuilder.addConditionAsGenQueryField(RodsGenQueryEnum.COL_DATA_NAME, QueryConditionOperators.EQUAL,
				testFileName);
		...
		IRODSGenQueryFromBuilder query = queryBuilder.exportIRODSQueryFromBuilder(100);
		...
		Assert.assertFalse("no result expected", resultSet.getResults().size() > 0);
	...
	public final void testExecuteMetadataQuerySoundsLikeBug179() throws Exception {
	...
	/**
	 * Ask for a row count in the query results feature: [#1046] Add total row count
	 * to gen query out
	 *
	 * @throws Exception
	 */
	...
	public final void testExecuteQueryWithRowCount() throws Exception {
		...
		builder.addSelectAsAgregateGenQueryValue(RodsGenQueryEnum.COL_COLL_ID, SelectFieldTypes.COUNT)
				.addConditionAsGenQueryField(RodsGenQueryEnum.COL_D_OWNER_ZONE, QueryConditionOperators.EQUAL,
						irodsAccount.getZone().trim());

		IRODSGenQueryFromBuilder query = builder.exportIRODSQueryFromBuilder(1);

		AbstractIRODSQueryResultSet resultSet = irodsGenQueryExecutor.executeIRODSQuery(query, 0);
	 ...
	 */
	@Test
	public final void testExecuteIRODSQueryBuilderQueryWithNumericCondition() throws Exception {
		...
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH + "/" + subdir);

		IRODSAccount irodsAccount = testingPropertiesHelper.buildIRODSAccountFromTestProperties(testingProperties);
		IRODSAccessObjectFactory accessObjectFactory = irodsFileSystem.getIRODSAccessObjectFactory();

		String targetIrodsFile = testingPropertiesHelper.buildIRODSCollectionAbsolutePathFromTestProperties(
				testingProperties, IRODS_TEST_SUBDIR_PATH + '/' + subdir);

		IRODSFile targetSubdir = accessObjectFactory.getIRODSFileFactory(irodsAccount)
		...
		// file1 small
		String localFileName = FileGenerator.generateFileOfFixedLengthGivenName(absPath, file1, smallSize);

		File localFile = new File(localFileName);
		dto.putOperation(localFile, targetSubdir, null, null);

		// file2 small
		localFileName = FileGenerator.generateFileOfFixedLengthGivenName(absPath, file2, smallSize);

		localFile = new File(localFileName);
		dto.putOperation(localFile, targetSubdir, null, null);

		// file3 large
		localFileName = FileGenerator.generateFileOfFixedLengthGivenName(absPath, file3, largeSize);

		localFile = new File(localFileName);
		dto.putOperation(localFile, targetSubdir, null, null);

		// file4 large
		localFileName = FileGenerator.generateFileOfFixedLengthGivenName(absPath, file4, largeSize);
		...
		builder.addSelectAsGenQueryValue(RodsGenQueryEnum.COL_COLL_NAME)
				...
				.addConditionAsGenQueryField(RodsGenQueryEnum.COL_DATA_SIZE,
						QueryConditionOperators.NUMERIC_GREATER_THAN_OR_EQUAL_TO, smallSize)
				.addConditionAsGenQueryField(RodsGenQueryEnum.COL_DATA_SIZE,
						QueryConditionOperators.NUMERIC_LESS_THAN_OR_EQUAL_TO, largeSize);

		IRODSGenQueryFromBuilder query = builder.exportIRODSQueryFromBuilder(50);

		IRODSQueryResultSetInterface resultSet = irodsGenQueryExecutor.executeIRODSQuery(query, 0);

		Assert.assertEquals("should get all 4 files", 4, resultSet.getResults().size());
		...
		Assert.assertEquals("should get 2 files", 2, resultSet.getResults().size());
		...
		Assert.assertEquals("should get 0 files", 0, resultSet.getResults().size());

	}

	/**
	 * Bug: [#1041] [iROD-Chat:8903] Jargon-core: query builder / SQL operators This
	 * is an iRODS bug being fixed in iRODS 3.2+
	 *
	 * @throws Exception
	 */
	@Ignore
	public final void testExecuteIRODSQueryBuilderQueryWithNumericGreaterThanOrEquaToBug1041() throws Exception {
