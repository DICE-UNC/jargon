...
public class LocalFileGzipCompressorTest {
	...
	public void testUnzipAGzipFile() throws Exception {
		...
		FileGenerator.generateManyFilesAndCollectionsInParentCollectionByAbsolutePath(localCollectionAbsolutePath,
				rootCollection, 2, 3, 2, "testFile", ".txt", 3, 2, 1, 200);
		...
		File zippedFile = localFileGzipCompressor.compress(tarredFile.getAbsolutePath());

		tarredFile.delete();
		File unzippedFile = localFileGzipCompressor.uncompress(zippedFile.getAbsolutePath());

		Assert.assertNotNull("no file", unzippedFile);
		Assert.assertTrue("unzippedFile does not exist", unzippedFile.exists());
		Assert.assertEquals("name should have .tar", targetTarFile, unzippedFile.getName());
	...
	public void testTarAndGzip() throws Exception {
		...
		Assert.assertEquals("name should have .gzip appended", targetTarFile + ".gzip", zippedFile.getName());
