...
public class ConnectionCreatingPoolableObjectFactoryTest {
	...
	public void testConnectionCreatingPoolableObjectFactoryNullAccount() {
		...
		new ConnectionCreatingPoolableObjectFactory(irodsAccount, irodsFileSystem.getIrodsSession(),
	...
	public void testMakeObject() throws Exception {
		...
		ConnectionCreatingPoolableObjectFactory factory = new ConnectionCreatingPoolableObjectFactory(irodsAccount,
				irodsFileSystem.getIrodsSession(), irodsFileSystem.getIrodsProtocolManager());
		PooledObject conn = factory.makeObject();
	 ...
	 *
	 * Runnable connRunnable = new Runnable() { public void run() { int randomInt =
	 * randomGenerator.nextInt(3000); try { Thread.sleep(randomInt); Object conn =
	 * factory.makeObject(); Assert.assertNotNull("null connection returned", conn);
	 * randomInt = randomGenerator.nextInt(3000); Thread.sleep(randomInt);
	 *
	 *
	 ...
	 * conn instanceof IRODSCommands; Assert.assertTrue("did not get a commands",
	 * isCommand); factory.destroyObject(conn); IRODSCommands actual =
	 * (IRODSCommands) conn;
