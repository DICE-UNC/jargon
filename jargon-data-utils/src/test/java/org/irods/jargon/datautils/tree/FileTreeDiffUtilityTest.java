...
public class FileTreeDiffUtilityTest {
	...
	public void testFileTreeDiffIrodsPlusOneDir() throws Exception {
		...
		if (children.length > 1) {
			...
			TestCase.fail("test setup failed, no children");
		}
	...
	private void compareFileTreeToNodesForDirMatchesAndNoDiffs(File[] files, Enumeration nodes) {
		for (File child : files) {
			if (child.isDirectory()) {
				if (nodes.hasMoreElements()) {
					FileTreeNode fileTreeNode = (FileTreeNode) nodes.nextElement();
					FileTreeDiffEntry fileTreeDiffEntry = (FileTreeDiffEntry) fileTreeNode.getUserObject();
					TestCase.assertEquals("nodes out of synch", child.getAbsolutePath(), fileTreeDiffEntry.getCollectionAndDataObjectListingEntry().getFormattedAbsolutePath()); 
					TestCase.assertEquals("node is not a no-diff directory entry", FileTreeDiffEntry.DiffType.DIRECTORY_NO_DIFF, fileTreeDiffEntry.getDiffType());
					...
					TestCase.fail("node is out of synch (missing) for file:" + child.getAbsolutePath());
				}
			}
		}
	}
	
	private void compareFileTreeToNodesForDirMatchesAndExpectADiff(File[] files, Enumeration nodes, FileTreeDiffEntry.DiffType diffType, String fileName) {
		for (File child : files) {
			if (child.isDirectory()) {
				if (nodes.hasMoreElements()) {
					FileTreeNode fileTreeNode = (FileTreeNode) nodes.nextElement();
					...
					if (fileTreeDiffEntry.getDiffType() != DiffType.DIRECTORY_NO_DIFF) {
						if (fileTreeDiffEntry.getDiffType() != diffType) {
							TestCase.fail("unexpectedDiffType");
						}
						if (fileTreeDiffEntry.getCollectionAndDataObjectListingEntry().getFormattedAbsolutePath().indexOf(fileName) == -1) {
							TestCase.fail("a file name that I didn't anticipate was found to have a diff errror");
						}
					}
