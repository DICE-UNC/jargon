...
public abstract class AbstractIRODSVisitor<E> {
	...
	/**
	 * Invoke an action based on processing an individual element in a collection.
	 * This is equivalent to iterating over a collection or query result
	 *
	 * @param visited
	 *            object of type <E> that is provided for a single instance of the
	 *            given collection of data to be processed by the visitor
	 ...
	 *            {@link IRODSAccount} and {@link IRODSAccessObjectFactory} objects
	 *            that may be used to further interact with iRODS. Note that such
	 *            interaction should be done on a separate thread so as to segregate
	 *            connections.
	 * @return a {@code VisitorDesiredAction} enum value that can signal the
	 *         client's desire to continue, halt, or take other actions
	 * @throws JargonException
	 */
	public abstract VisitorDesiredAction invoke(final E visited,
			final AbstractIRODSVisitorInvoker<E> abstractIRODSVisitorInvoker) throws JargonException;

	/**
	 * Final call from invoker when all items have been processed, signalling a
	 * successful (non cancelled) completion of processing for the given collection
	 * of data
	 *
	 * @throws JargonException
	 */
