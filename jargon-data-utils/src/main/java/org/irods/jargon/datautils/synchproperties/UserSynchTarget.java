...
/**
 * Describes an iRODS Synch target for a device. A target is essentially a directory on iRODS that relates to a
 * corresponding directory on the local file system of the target device. This synch information is applicable to one
 * iRODS zone.
 * <p/>
 * iRODS stores information about this synch folder, including the last synchronization time stamps from the perspective
 * of both the device, and iRODS, so that differences can be noted.
 * 
...
public class UserSynchTarget {
    ...
    /**
     * Absolute path to the top level collection that represents the iRODS synch data
     */
    ...
    /**
     * Time stamp from the local clock with the last synchronization point, or 0 if no synch has been done
     */
    private long lastLocalSynchTimestamp = 0;

    /**
     * Time stamp with the irods clock with the last synchronization point, or 0 if no synch has been done
     */
    ...
    public void setLastIRODSSynchTimestamp(long lastIRODSSynchTimestamp) {
