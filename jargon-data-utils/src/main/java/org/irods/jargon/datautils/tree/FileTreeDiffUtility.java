...
public interface FileTreeDiffUtility {
	 ...
	 * iRODS time stamps are not a good indicator for direct comparison, the utility
	 * compares two file (data objects) based on length. Optionally, the iRODS
	 * timestamp can be compared to a provided timestamp value to treat files
	 * modified after the given cut-off as 'changed'.
	 * <p>
	 ...
	 *            compared relative to the respective left hand and right hand root
	 *            absolute paths
	 ...
	 *         that contain the {@link FileTreeDiffEntry} as the user data object.
