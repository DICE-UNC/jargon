...
/**
 * <code>Runnable</code> meant to run in a thread, processing the given transfer.
 * 
...
public class TransferRunner implements Runnable {
    ...
    private static final Logger log = LoggerFactory.getLogger(TransferRunner.class);

    /**
     * Constructs a process to transfer data to/from iRODS.
     * 
     * @param transferManager
     *            {@link TransferManager} that coordinates the transfer of data, and manages callbacks and
     *            notifications.
     * @param localIRODSTransfer
     *            {@link LocalIRODSTransfer} that contains data describing the type of transfer and the status.
     * @param transferControlBlock
     *            {@link TransferControlBlock} maintains shared references to data about the transfer between the client
     *            asking for the transfer, and the transfer process.
     * @param transferQueueService
     *            {@link TransferQueueService} that manages persistence and access to the transfer data.
     * @throws JargonException
     */
    public TransferRunner(final TransferManager transferManager, final LocalIRODSTransfer localIRODSTransfer,
    ...
    public void run() {
        ...
        try {
            irodsLocalTransferEngine = IRODSLocalTransferEngine.instance(transferManager, transferControlBlock,
        ...
        try {
            ...
            /*
             * I have tried the transfer. If any errors had occurred, the best effort was made to log the error in the
             * transfer database, and in case of error would have returned or thrown an exception. I can assume that the
             * transfer went correctly and the database was updated in the irodsLocalTransferEngine.
             * processPutOperation() method.
             */
            ...
            log.error(
                    ...
                    je);
            try {
                transferQueueService.markTransferAsErrorAndTerminate(localIRODSTransfer, je, transferManager);
     ...
     */
    private void handleErrorCreatingTransferEngineInstance(final TransferQueueService transferQueueService,
        ...
        try {
            ...
            log.error("error marking transfer as an error, cannot update queue with this status", e);
