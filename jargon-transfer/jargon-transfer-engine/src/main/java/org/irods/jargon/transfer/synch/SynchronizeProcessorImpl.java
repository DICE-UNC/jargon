...
/**
 * Compare a local watched folder to a remote iRODS folder and enqueue necessary transfers to synchronize between the
 * two.
 * 
...
public class SynchronizeProcessorImpl implements SynchronizeProcessor {
    ...
    private static final Logger log = LoggerFactory.getLogger(SynchronizeProcessorImpl.class);
     ...
     * (non-Javadoc)
     * 
     * @see org.irods.jargon.transferengine.synch.SynchronizeProcessor# synchronizeLocalToIRODS(java.lang.String,
     * java.lang.String, java.lang.String, long)
     */
    @Override
    public void synchronizeLocalToIRODS(final String synchDeviceName, final String localRootAbsolutePath,
            final String irodsRootAbsolutePath, final long timestampOfLastSynch) throws JargonException {
        ...
        FileTreeModel diffModel = fileTreeDiffUtility.generateDiffLocalToIRODS(
                new File(calculatedLocalRoot.toString()), calculatedIrodsRoot.toString(), timestampOfLastSynch);
        log.debug("diff model obtained");
        if (diffModel == null) {
            throw new JargonException("null diff model returned, cannot process");
        }

        processDiff((FileTreeNode) diffModel.getRoot(), calculatedLocalRoot.toString(), calculatedIrodsRoot.toString(),
                timestampOfLastSynch);
        log.debug("processing complete");
    }

    // given a tree model, do any necessary operations to synchronize
    private void processDiff(FileTreeNode diffNode, String localRootAbsolutePath, String irodsRootAbsolutePath,
        ...
        if (fileTreeDiffEntry.getDiffType() == DiffType.DIRECTORY_NO_DIFF) {
            log.debug("evaluating directory: {}", fileTreeDiffEntry.getCollectionAndDataObjectListingEntry()
            ...
            log.debug("local file is new directory {}", fileTreeDiffEntry.getCollectionAndDataObjectListingEntry()
                    .getFormattedAbsolutePath());
            ...
            log.debug("irods file is new directory {}", fileTreeDiffEntry.getCollectionAndDataObjectListingEntry()
                    .getFormattedAbsolutePath());
            ...
            log.warn("file directory/name collision on:{}", fileTreeDiffEntry.getCollectionAndDataObjectListingEntry()
                    .getFormattedAbsolutePath());
            // TODO: figure out the disposition of this type of error. should this go back to the user in the transfer
            // panel?
        } else {
    ...
    private void processOutOfSynchOfFile(FileTreeNode diffNode, String localRootAbsolutePath,
            String irodsRootAbsolutePath) {
        // TODO Auto-generated method stub

    }

    private void scheduleIrodsToLocal(FileTreeNode diffNode, String localRootAbsolutePath, String irodsRootAbsolutePath) {
        // TODO Auto-generated method stub
     ...
     */
    private void scheduleLocalToIrods(FileTreeNode diffNode, String localRootAbsolutePath, String irodsRootAbsolutePath)
            throws JargonException {
        /*
         * the diff node will have the absolute local path of the file, this is the source of the put. the iRODS path
         * will be the local path minus the local root, appended to the iRODS root
         */
        ...
        transferManager.enqueueAPut(entry.getFormattedAbsolutePath(), sb.toString(),
    ...
    @Override
    public void setFileTreeDiffUtility(final FileTreeDiffUtility fileTreeDiffUtility) {
    ...
    private void checkInitialization() {
        ...
        if (irodsAccessObjectFactory == null) {
            throw new IllegalStateException("no irodsAccessObjectFactory was set");
        }
