...
/**
 * Service to manage a transfer from a client to an iRODS server via put , replicate, or get. This engine will monitor
 * transfers and maintain status information for reporting, restarting, and other management aspects, reporting this
 * information to the <code>TransferManager</code>.
 * 
 * Note that this class is meant to receive and process callbacks from a single transfer process. If, in the future,
 * multiple simultaneous transfers are implemented, there will need to be one transfer engine per transfer operation.
 * This class needs to maintain a reference to the current transfer so that it may properly associate callbacks from the
 * actual transfer with the specfic enqueued transfer operation, which is held in the <code>currentTransfer</code>
 * instance variable.
 * 
...
final class IRODSLocalTransferEngine implements TransferStatusCallbackListener {
    ...
    private final Logger log = LoggerFactory.getLogger(IRODSLocalTransferEngine.class);
    ...
    private static IRODSLocalTransferEngine instance = null;

    /**
     * Static initializer to create transfer engine. By default, successful transfers are to be logged.
     * 
     * @param irodsAccessObjectFactory
     *            <code>IRODSAccessObjectFactoryImpl</code> that provides access to objects that interact with iRODS.
     * @param transferControlBlock
     *            <code>TransferControlBlock</code> implementation that provides a common communications object between
     *            the manager and the actual transfer process. For example, this block has a method to communicate a
     *            cancellation from the manager to the running transfer. This block also can be implemented to provide a
     *            filter to select files to transfer, such as a restart filter.
     * @return <code>IRODSLocalTransferEngine</code> instance.
     * @throws JargonException
     */
    public static IRODSLocalTransferEngine getInstance(final TransferManagerImpl transferManager,
        ...
        if (instance == null) {
            instance = new IRODSLocalTransferEngine(transferManager, transferControlBlock, true);
        }
        return instance;
    }
     ...
     *            <code>boolean</code> that indicates whether successful transfers are logged. This can be turned off by
     *            setting to <code>false</code>, useful for very large transfers where local database overhead is an
     *            issue.
     * @return <code>IRODSLocalTransferEngine</code> instance.
     * @throws JargonException
     */

    protected static IRODSLocalTransferEngine instance(final TransferManagerImpl transferManager,
            final TransferControlBlock transferControlBlock, final boolean logSuccessfulTransfers)
            throws JargonException {
        if (transferManager == null) {
            throw new JargonException("transferManager is null");
        }
        ...
        if (instance == null) {
            instance = new IRODSLocalTransferEngine(transferManager, transferControlBlock, logSuccessfulTransfers);
        }
        return instance;
    }
     ...
     *            <code>LocalIRODSTransfer</code> that has information to accomplish a transfer of data to or from
     *            iRODS.
     * @throws JargonException
     */
    protected synchronized void processOperation(final LocalIRODSTransfer localIrodsTransfer) throws JargonException {
        ...
        final IRODSAccount irodsAccount = IRODSAccount.instance(localIrodsTransfer.getTransferHost(),
                localIrodsTransfer.getTransferPort(), localIrodsTransfer.getTransferUserName(),
                HibernateUtil.retrieve(localIrodsTransfer.getTransferPassword()), "/",
                localIrodsTransfer.getTransferZone(), localIrodsTransfer.getTransferResource());
        ...
        IRODSFileSystem irodsFileSystem = IRODSFileSystem.instance(); // FIXME:
                                                                      ...
                                                                      // not
                                                                      // instanciated
        final DataTransferOperations dataTransferOperations = irodsFileSystem.getIRODSAccessObjectFactory()
        ...
        try {
            ...
            if (aborted == true) {
                ...
                markTransferException(wrapUpTransfer, transferException);
                    ...
                    && (wrapUpTransfer.getTransferState().equals(TransferState.PAUSED) || wrapUpTransfer
            ...
            log.info("wrap up of finished transfer at update time {}", wrapUpTransfer);
    ...
    private void markTransferWasAborted(final LocalIRODSTransfer wrapUpTransfer) throws JargonException {
        wrapUpTransfer.setTransferStatus(org.irods.jargon.transfer.dao.domain.TransferStatus.WARNING);
     ...
     * @throws JargonException
     */
    private JargonException transferTypePut(final LocalIRODSTransfer localIrodsTransfer,
            final DataTransferOperations dataTransferOperations, final IRODSFileFactory irodsFileFactory)
            throws JargonException {

        final IRODSFile targetFile = irodsFileFactory.instanceIRODSFile(localIrodsTransfer.getIrodsAbsolutePath());
        targetFile.setResource(localIrodsTransfer.getTransferResource());
        final File localFile = new File(localIrodsTransfer.getLocalAbsolutePath());
     ...
     * @throws JargonException
     */
    private JargonException transferTypeGet(final LocalIRODSTransfer localIrodsTransfer,
            final DataTransferOperations dataTransferOperations, final IRODSFileFactory irodsFileFactory)
            throws JargonException {

        final IRODSFile sourceFile = irodsFileFactory.instanceIRODSFile(localIrodsTransfer.getIrodsAbsolutePath());
        ...
        try {
            dataTransferOperations.getOperation(sourceFile, localFile, this, transferControlBlock);
        } catch (JargonException je) {
            log.error("exception in transfer will be marked as a global exception, ending the transfer operation");
            transferException = je;
        }
     ...
     * @throws JargonException
     */
    private JargonException transferTypeReplicate(final LocalIRODSTransfer localIrodsTransfer,
            final DataTransferOperations dataTransferOperations, final IRODSFileFactory irodsFileFactory)
            throws JargonException {
        final IRODSFile targetFile = irodsFileFactory.instanceIRODSFile(localIrodsTransfer.getIrodsAbsolutePath());
        targetFile.setResource(localIrodsTransfer.getTransferResource());
        ...
        try {
            dataTransferOperations.replicate(localIrodsTransfer.getIrodsAbsolutePath(),
                    localIrodsTransfer.getTransferResource(), this, transferControlBlock);
        } catch (JargonException je) {
            log.error("exception in transfer will be marked as a global exception, ending the transfer operation");
            transferException = je;
        }
     ...
     * @throws JargonException
     */
    private void evaluateTransferErrorsPerFile(final LocalIRODSTransfer localIrodsTransfer) throws JargonException {

        synchronized (this) {
            if (transferControlBlock.getErrorCount() > 0
                    && transferControlBlock.getErrorCount() < transferControlBlock.getMaximumErrorsBeforeCanceling()) {
                localIrodsTransfer.setTransferStatus(org.irods.jargon.transfer.dao.domain.TransferStatus.WARNING);
                localIrodsTransfer.setTransferState(TransferState.COMPLETE);
                transferManager.notifyWarningCondition();
            } else if (transferControlBlock.getErrorCount() > 0) {
                localIrodsTransfer.setTransferStatus(org.irods.jargon.transfer.dao.domain.TransferStatus.ERROR);
                localIrodsTransfer.setTransferState(TransferState.COMPLETE);
                transferManager.notifyErrorCondition();
            } else {
                localIrodsTransfer.setTransferStatus(org.irods.jargon.transfer.dao.domain.TransferStatus.OK);
                localIrodsTransfer.setTransferState(TransferState.COMPLETE);
            }
        }
    }
     ...
     * @throws JargonException
     */
    private void markWarningZeroFilesTransferred(final LocalIRODSTransfer localIrodsTransfer) throws JargonException {
     ...
     * @throws JargonException
     */
    private void markTransferException(final LocalIRODSTransfer localIrodsTransfer,
     ...
     * (non-Javadoc)
     * 
     * @see org.irods.jargon.core.transfer.TransferStatusCallbackListener#statusCallback
    ...
    public void statusCallback(final TransferStatus transferStatus) throws JargonException {
        ...
        synchronized (this) {
            ...
            localIRODSTransferItem.setSourceFileAbsolutePath(transferStatus.getSourceFileAbsolutePath());
            localIRODSTransferItem.setTargetFileAbsolutePath(transferStatus.getTargetFileAbsolutePath());
    ...
    private void processStatusCallback(final TransferStatus transferStatus,
            final LocalIRODSTransferItem localIRODSTransferItem) throws HibernateException, JargonException {
        ...
        if (transferStatus.getTransferState() == TransferStatus.TransferState.FAILURE) {
            ...
            localIRODSTransferItem.setErrorMessage(transferStatus.getTransferException().getMessage());
        ...
        try {
            ...
            if (transferStatus.getTransferState() == TransferStatus.TransferState.SUCCESS) {
                ...
                mergedTransfer.setLastSuccessfulPath(transferStatus.getSourceFileAbsolutePath());
                mergedTransfer.setTotalFilesCount(transferStatus.getTotalFilesToTransfer());
                mergedTransfer.setTotalFilesTransferredSoFar(transferStatus.getTotalFilesTransferredSoFar());
            ...
            if (updateItemRequired) {
                ...
                mergedTransfer.getLocalIRODSTransferItems().add(localIRODSTransferItem);
            }
    ...
    protected synchronized void setCurrentTransfer(final LocalIRODSTransfer currentTransfer) {
