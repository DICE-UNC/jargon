...
public class TransferManagerTest {
    ...
    public void testEnqueueAPutWhenPaused() throws Exception {
        ...
        Assert.assertEquals("should be finished and idle", TransferManager.RunningStatus.PAUSED,
    ...
    public void testEnqueueAPutWithRestart() throws Exception {
        ...
        FileGenerator.generateManyFilesInGivenDirectory(IRODS_TEST_SUBDIR_PATH + '/' + rootCollection, "test", ".doc",
                20, 1, 2);
        ...
        Assert.assertTrue("not enough files generated for the test", localFiles.length > 3);
        ...
        try {
            TransferDAOManager transferDAOMgr = TransferDAOManager.getInstance();
        ...
        IRODSFile testDir = irodsFileSystem.getIRODSFileFactory(irodsAccount).instanceIRODSFile(
        ...
        for (File irodsFile : irodsFilesThatWerePut) {
            ...
            for (int i = 0; i < 2; i++) {
                if (irodsFile.getName().equals(localFiles[i].getName())) {
                    Assert.fail("file should not have been transferred:" + irodsFile.getAbsolutePath());
                }
            }
        }

        Assert.assertEquals("correct number of files were not transferred", localFiles.length - 3,
                irodsFilesThatWerePut.length);
    ...
    public void testEnqueueAPutTwice() throws Exception {
        ...
        List<LocalIRODSTransfer> transferQueue = transferManager.getCurrentQueue();
    ...
    public void enqueueAReplicate() throws Exception {
        ...
        while (true) {
            ...
            Thread.sleep(1000);
            if (transferManager.getRunningStatus().equals(TransferManager.RunningStatus.IDLE)) {
                break;
            }
        }
    ...
    public void enqueueAGet() throws Exception {
        ...
        transferManager.enqueueAGet(irodsCollectionRootAbsolutePath + "/" + rootCollection, localReturnedAbsolutePath,
