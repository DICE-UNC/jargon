 ...
 * @since 11/03/2009 testing of
 *        {@link org.irods.jargon.testutils.filemanip.ScratchFileUtils}
 */
...
public class ScratchFileUtilsTest {
	...
	public final void testCreateScratchDirWhenNotExists() throws Exception {
		...
		Assert.assertTrue("did not create scratch dir",
				scratchFileUtils.checkIfFileExistsInScratch(scratchFileName));
	}
	...
	public final void testCheckFileExistsUnderScratch() throws Exception {
		...
		boolean foundDir = scratchFileUtils
				.checkIfFileExistsInScratch(testingDir);
	...
	public final void testCreateAndReturnScratchPathToFileName()
		...
		scratchFileUtils.createAndReturnAbsoluteScratchPath(testingDir);
	...
	@Test
	public final void testCreateAndReturnAbsoluteScratchPathToFileName()
			throws Exception {
		Properties props = new Properties();
		...
		pathBuilder.append(testingDir);
		pathBuilder.append(testingFileName);
		String newPath = scratchFileUtils
				.createAndReturnAbsoluteScratchPath(testingDir);
		Assert.assertTrue("no path returned", newPath.length() > 0);

		// now create a file using the path
		FileGenerator.generateFileOfFixedLengthGivenName(newPath,
				testingFileName, 1);

		File existsFile = new File(newPath);
		Assert.assertTrue("did not find scratch file just added",
				existsFile.exists());
	}
	...
	public final void testComputeValidFileChecksum() throws Exception {
		...
		// now create a file using the path
		String fullPath = scratchFileUtils
				.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH);
		FileGenerator.generateFileOfFixedLengthGivenName(fullPath,
				testingFileName, 11);
