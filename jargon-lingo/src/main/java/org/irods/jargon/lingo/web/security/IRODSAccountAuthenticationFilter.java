...
/**
 * Web authentication filter for Spring Security enabled IRODS Account authorization
 * @author Mike Conway - DICE (www.irods.org)
 *
 */
public class IRODSAccountAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request,
		...
		log.info("authentication filter invocation, url=" +request.getRequestURI() );
		
		log.info("requires auth value:" + this.requiresAuthentication(request, response));
		...
		try {
			loginIRODSAccount = IRODSAccount.instance(host, port, user, password, b.toString(),
					zone, resource);
		...
		Authentication token  =   this.getAuthenticationManager().authenticate(new IRODSAuthenticationToken(loginIRODSAccount));
